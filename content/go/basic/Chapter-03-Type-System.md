...
func main() {
    ...
    fmt.Printf("string: %s\n", s) // string: Hello
    fmt.Printf("err 是不是 nil: %t\n", err == nil) // err 是不是 nil: true
}
...
func main() {
    ...
    var c Int1 = 30
    fmt.Println("c 是 Int1 类型，值 =", c) // c 是 Int1 类型，值 = 30
    fmt.Println("int(c) 才能和 int 一起运算:", int(c)+a) // int(c) 才能和 int 一起运算: 40
    // var d Int2 = 40
...
func main() {
    ...
    maxInt := int(^uint(0) >> 1)      // 最大 int
    minInt := -maxInt - 1             // 最小 int
    fmt.Printf("int 取值范围: %d 到 %d\n", minInt, maxInt) // int 取值范围: -9223372036854775808 到 9223372036854775807 (64位系统)
}
```
...
func main() {
    ...
    fmt.Printf("变量值: i16=%d, i32=%d, i64=%d\n", i16, i32, i64) // 变量值: i16=32767, i32=2147483647, i64=9223372036854775807
...
`uint` 和 `int` 一样，宽度取决于目标平台：32 位平台上是 32 位，64 位平台上是 64 位。用 `unsafe.Sizeof` 可以直接观察这一点——正因为如此，**不要把 `uint` 用在需要固定宽度的场合**（比如二进制协议或哈希值）。
...
func main() {
    ...
    fmt.Printf("uint64: 0 到 %d\n", uint64(math.MaxUint64)) // uint64: 0 到 18446744073709551615
    // uint64: 0 到 18446744073709551615

    fmt.Printf("变量值: u16=%d, u32=%d, u64=%d\n", u16, u32, u64) // 变量值: u16=65535, u32=4294967295, u64=18446744073709551615
...
无符号整数把减法结果「绕」回最大值，这是 Go 明确定义的行为（不是未定义行为），所以在做倒计时、环形缓冲区时可以直接依赖它：
...
`math.Inf(1)` 构造正无穷；浮点数一旦溢出到超出 `float64` 表示范围，也会得到正负无穷：
...
`math.Inf(-1)` 构造负无穷。注意不能用常量相除来制造无穷——`-1.0 / 0.0` 是编译期错误，因为除数是常量零：
...
func main() {
    // 正负无穷用 math.Inf 构造，不能用常量除以 0（那会直接编译报错）
    posInf := math.Inf(1)
    negInf := math.Inf(-1)
    fmt.Printf("正无穷: %v, 负无穷: %v\n", posInf, negInf) // 正无穷: +Inf, 负无穷: -Inf
}
```
...
func main() {
    ...
    fmt.Printf("累加 1000 次 0.1 = %.20f\n", sum) // 累加 1000 次 0.1 = 99.99999999999859312538（不是 100！）

    fmt.Printf("实际值 vs 期望值: %.20f vs 100.00000000000000000\n", sum) // 实际值 vs 期望值: 99.99999999999859312538 vs 100.00000000000000000
}
```
...
浮点数在二进制里大多是近似值，两次「看起来一样」的计算可能得到不同的二进制结果，于是 `==` 会返回 `false`：
...
`NaN` 和任何值（包括它自己）比较都不相等，不能用 `==` 判断；正负无穷则遵循正常的大小关系：
...
// absComplex 返回复数的模。注意 real/imag 返回的是 float32，
// 要先转成 float64 再做开方，否则精度会不必要地损失。
...
func main() {
    ...
    // 更复杂的运算可以交给 math/cmplx 标准库，
    // 例如指数函数 e^(1+0i) = e
    exp := cmplx.Exp(complex(1, 0))
...
复数可以在复平面上表示为一个点或向量——实部是横坐标，虚部是纵坐标：
...
func main() {
    ...
    // cmplx.Abs 求模：|3+4i| = √(3²+4²) = 5
    fmt.Printf("该点到原点的距离（模）= %.2f\n", cmplx.Abs(c)) // 该点到原点的距离（模）= 5.00
}
```
...
复数的加减乘除和实数一样直接用运算符写，模、共轭、指数等运算则交给 `math/cmplx` 包：
...
把字符串转成 `[]rune` 之后，下标访问得到的就是**码点**而不是字节，一个汉字也只占一个位置：
...
`*T` 表示「指向 T 的指针」，`&x` 取地址，`*p` 解引用。下面是这三个符号最基本的用法：
...
Go 要求所有类型转换都显式写出目标类型，没有隐式转换。整型之间互相转换不会报错，但可能丢数据：
...
浮点转整数是**截断**（向零取整），不是四舍五入；超出目标类型范围时行为是实现相关的，不要依赖：
...
把一组典型的小数转成整数，可以直观地看到「向零截断」这条规则：
...
`[]byte(s)` 会复制一份新的字节序列，改它不会影响原字符串——这也正是「字符串不可变」的实现方式：
...
`[]rune(s)` 按码点切分，适合做「按字符处理」的逻辑，比如统计字数、翻转字符串：
...
`type Age int` 创建的是一个**全新类型**，它拥有 int 的底层表示，但不等于 int：
...
`type Age = int` 只是给 int 起了个小名，二者完全等价，可以互相赋值：
...
func main() {
    ...
    fmt.Println("Age 变量的值:", a) // Age 变量的值: 25
...
无类型常量（untyped constant）在赋值或使用时才确定类型，这也是为什么 `const a = 42` 既能赋给 int 也能赋给 float64：
...
`:=` 根据右边的值推断类型，规则和常量快照一致，但变量一旦确定类型就不能再变：
...
函数名后面的 `(result int)` 给返回值起了名字，函数体里可以直接给它赋值，最后的 `return` 不需要写参数：
...
调用泛型函数时通常不用写方括号里的类型，编译器会根据实参自动推断：
...
除了类型相同，Go 还允许几种**可赋值**的情形，其中最常用的是「具体类型可以赋给它能满足的接口」：
...
布尔、数值、字符串、指针、通道、接口以及由它们组成的数组和结构体都可以用 `==` 比较；切片、映射和函数则不行：
...
转换必须写成 `T(x)` 的形式。数值之间可以随意转（可能丢精度），字符串和数值之间则**不能**直接转，得借助 `strconv`：
...
「零值」是变量声明后默认得到的值；「空值」是另一种状态。最容易混淆的是字符串和切片：
...
func main() {
    ...
    fmt.Printf("string 零值: %q\n", s1) // string 零值: ""
    fmt.Printf("是不是空串: %t\n", s1 == "") // 是不是空串: true
    fmt.Printf("string 空字符串: %q, 是不是空串: %t\n", s2, s2 == "") // string 空字符串: "", 是不是空串: true
    // 注意：字符串是值类型，写 s1 == nil 会直接编译报错（cannot compare string to nil）
