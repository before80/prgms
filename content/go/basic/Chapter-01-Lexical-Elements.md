...
- **2 字节**：拉丁字母扩展、希腊字母、西里尔字母、希伯来字母、阿拉伯字母等"中量级选手"（码点 U+0080 到 U+07FF）
  - 注意：这一档**不包括**汉字和日文假名
  - 比如 'é'（带重音的 e）就是 2 个字节
  - 比如 'Ω' (U+03A9)、'Ж' (U+0416) 也都是 2 个字节

- **3 字节**：中日韩文字等"重量级选手"（码点 U+0800 到 U+FFFF）
  - 比如 '中' (U+4E2D) 就是 3 个字节
  - 比如常用汉字、日文假名、韩文音节，以及 ☀ (U+2600)、✅ (U+2705) 这类较早被收录的符号都是 3 个字节

- **4 字节**：扩展区的字符（码点 U+10000 到 U+10FFFF）
  - 比如大部分现代 emoji（🎉 U+1F389、🌍 U+1F30D、😀 U+1F600）都是 4 个字节
  - 比如某些生僻/考古发现的汉字（如 𠀀 U+20000）
...
flowchart TB
    ...
    D --> H[拉丁字母扩展<br/>希腊文<br/>西里尔文<br/>希伯来文]
    E --> I[中日韩文字<br/>日文假名<br/>☀ ✅ 等符号]
    F --> J[大部分现代emoji<br/>生僻汉字]
...
> 下面这个例子涉及**两个包**，没法作为单个 `main.go` 编译，所以用文本形式并列展示两个文件的内容：

```text
// ===== 文件：mypackage/mypackage.go =====
package mypackage

// ExportedFunction 首字母大写 → 可以被其他包引用
func ExportedFunction() string { return "你好，我来自 mypackage 包！" }

// unexportedFunction 首字母小写 → 只能在本包内使用
func unexportedFunction() string { return "我是内部实现，外部看不到" }
...
func main() {
    ...
    // 但无法访问未导出的函数——编译直接报错！
    // fmt.Println(mypackage.unexportedFunction()) // ❌ 编译错误：cannot refer to unexported name
}
```
...
> **历史彩蛋**：`any` 和 `comparable` 是在 **Go 1.18** 随泛型一起引入的（不是 1.21）。`any` 是 `interface{}` 的别名，写起来更短；`comparable` 则是一个预声明的约束接口，表示"支持 `==` 和 `!=` 比较"的类型集合。而 **Go 1.21** 新增的其实是三个内置函数 `min`、`max`、`clear`，别把这两批特性记混了。`any` 就像一把万能钥匙，能打开任何类型的门，让泛型代码读起来更自然。
...
把这张表里的关键字加起来正好是 25 个：4 + 2 + 4 + 7 + 5 + 3 = 25。注意不要重复计算 `type`——它只属于"声明"这一类。
...
> **有趣观察**：如果你写过 Java 或者 C++，你可能注意到 Go 的声明关键字非常少。Java 有 `public static void main(String[] args)` 这种让人眼前一黑的写法，而 Go 只需要 `func main()`——干净利落。这大概就是 Go 的设计哲学："less is more"（少即是多）。Go 的作者之一 Rob Pike 说过："简洁性是 Go 的核心价值观之一。"
...
func main() {
    ...
    // 短路求值：用一个有“副作用”的辅助函数来观察右侧到底有没有被执行
    check := func(name string) bool {
        fmt.Println("  执行了检查:", name)
        return true
    }

    fmt.Println("---短路求值演示---")
    // && 左边是 false，右边根本不会被调用
    fmt.Println("false && check(\"A\") =", false && check("A")) // false（没有输出“执行了检查: A”）
    // || 左边是 true，右边同样不会被调用
    fmt.Println("true  || check(\"B\") =", true || check("B"))  // true（没有输出“执行了检查: B”）
    // 只有必须求值右侧时才会真的调用
    fmt.Println("true  && check(\"C\") =", true && check("C"))  // 执行了检查: C → true
}
...
|   // 按位或（OR）
^   // 按位异或（XOR）
&^  // 位清除（AND NOT），常用于操作标志位：把一个位掩码里指定的位清零
<<  // 左移
>>  // 右移
```
...
// 注意：<- 不在这张表里。它是一元运算符（接收操作），
// 优先级比上面所有二元运算符都高，比如 <-ch + 1 等价于 (<-ch) + 1。
```
...
func main() {
    ...
    fmt.Println("警报声:\a") // 警报声:（终端会发出一声提示音）
}
...
> 同样，这里需要 `geometry` 包一起才能运行，所以用文本形式给出两个文件：

```text
// ===== 文件：geometry/geometry.go =====
package geometry
...
func main() {
    ...
    // 行长度：gofmt 不会替你折行——太长就该自己拆开
    veryLongFunctionName := func(param1, param2, param3 int) int {
        return param1 + param2 + param3
    }
    fmt.Println("三个数之和:", veryLongFunctionName(1, 2, 3)) // 三个数之和: 6
    ...
    // 空格：二元运算符前后要加空格
    sum := 1 + 2 // ✅
    // sum := 1+2   // ❌（能编译，但不符合 gofmt 规范）
    product := 3 * 4 // ✅
    fmt.Println(sum, product) // 3 12
    ...
    // 括号：if 的条件不需要括号，gofmt 会把多余的括号删掉
    if sum > 0 {
        fmt.Println("正数") // 正数
    }
}
...
```text
// ✅ 好的包名（简短、小写、单个单词、全小写，不要下划线和驼峰）
package strings
package io
package http

// ❌ 不好的包名
package StringHelper   // 驼峰 + 含义含糊
package MyUtil         // 同上
package common_utils   // 下划线不符合 Go 命名习惯
```

> 这段只是为了并列展示命名风格，不能作为单独的文件编译——一个 `.go` 文件里只能有一个 `package` 声明。
...
var AppName string = "MyApp" // 导出变量：首字母大写，其他包也能用
var maxSize int = 100        // 未导出：首字母小写，只有本包能用
...
// 多方法接口：把已有的接口嵌进来组合，这里直接用标准库的 io.Reader / io.Writer
...
在一个文件里，常见的声明顺序是：常量 → 变量 → 类型 → 构造函数/方法 → 其他函数。下面的例子就是按这个顺序排的。
...
// ✅ 好的注释（解释"为什么"）
// 这里用互斥锁而不是原子操作，
// 因为要保证"先检查再修改"这一整段逻辑不会被别的 goroutine 插队。
var mu sync.Mutex
