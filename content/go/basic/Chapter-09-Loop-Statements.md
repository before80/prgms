...
`for { ... }` 就是不带任何条件的循环，会一直转下去，只能靠内部的 `break` 或 `return` 退出。它是“服务器主循环”“事件循环”的标准写法：
...
把 `for` 当作 `while` 用：只有一个条件表达式，没有初始化和步进语句：
...
`for range` 是遍历数组、切片、map、字符串、channel 的统一写法。这一节汇总它的各种形态：
...
多个切片长度相同时，用同一个下标同时遍历它们——注意要取长度的最小值，否则会越界：
...
Go 的 `range` 只能正着走，倒着遍历必须自己控制下标。要特别小心下标是 `int`，用 `i >= 0` 作条件才不会死循环：
