...
> 实际编译时，Go 会报 `label label not defined`。原因是**标签的作用域只在它所在的函数内**，`main` 里根本看不到 `other` 里的 `label`，所以跨函数 `goto` 从语言层面就不可能成立。
...
上面这段代码是**正确的**：`goto` 从循环内部跳到循环外的标签属于「向外跳」，完全合法；真正被禁止的是反方向的「跳进代码块内部」（见限制二）。

> 换句话说，限制针对的是「往里跳」，而不是「往外跳」。
...
skip:
    fmt.Println("跳到循环外部") // 跳出两层循环后继续执行
}
```
...
func main() {
label: // ❌ 编译错误：label label defined and not used
    fmt.Println("用不到的标签")
}
```

上面这段代码也是**错误的**——Go 编译器会报 `label label defined and not used`。标签不像注释，它不是"写给自己看的"，定义了就一定得有 `goto` / `break` / `continue` 真的用到它，否则编译器会直接拒绝编译。

**为什么要有这些限制？**

这些限制的目的很简单：**防止代码变成"意大利面条"**。

没有限制的 `goto` 可以让你的代码在任何地方跳来跳去，今天跳到这里，明天跳到那里，用不了多久，你的代码就会变成一团乱麻，连你自己都看不懂。Go 语言的设计者之所以加上这些限制，就是为了让 `goto` 只能在一个「合理」的范围内使用。
...
需要"失败就从头再来"时，`goto` 能把重试写成一个直白的跳回动作，不必再引入一个外层的 `for` 循环和标志变量：
...
func main() {
    fmt.Println("result =", compute(3, 4)) // result = 11
}
```
