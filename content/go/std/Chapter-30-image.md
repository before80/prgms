...
graph TD
    subgraph cs["图像坐标系"]
        direction TB
        O["原点 (0, 0)"] --> W["X 轴：向右增大<br/>对应宽度 width"]
        O --> H["Y 轴：向下增大<br/>对应高度 height"]
    end

    subgraph pt["点与矩形"]
        P1["Point(X: 2, Y: 1)"] -->|"落在矩形内"| R1["Rectangle(Min: 0,0  Max: 5,5)"]
    end
```

> 记牢这条约定：**图像坐标的原点在左上角，Y 轴向下增大**。所以 `Point{Y: 1}` 比 `Point{Y: 3}` 更靠上——这一点和数学课上的平面直角坐标系正好相反，也是写裁剪、缩放代码时最容易搞反的地方。
...
> 说明：上面每一行是一个像素的四个分量（R、G、B、A），8 个像素一共占 32 个字节。行 1 的四个像素依次是红、绿、蓝、黄；行 2 是黑、白、灰、半透明的红——对应前面 `Pix` 数组里连续排列的 32 个字节。
