...
准确地说，限制来自**输出格式**：代码分割产出的每个 chunk 都是独立的 ES Module，彼此之间要靠静态 `import` / 动态 `import()` 以及 `import.meta.url` 来互相引用和定位，这套机制只有 ESM 输出才具备。所以 esbuild 只允许 `format: 'esm'` 时开启 `splitting`——写成 `format: 'cjs'` 或 `format: 'iife'` 再打开 `splitting`，构建会直接报错。

顺带澄清一个常见误解：**Node.js 本身早就支持 ESM**，所以"Node 项目不能代码分割"的说法是不对的，能不能分割取决于你让 esbuild 输出什么格式，以及运行环境是否支持 ESM。
...
### 6.1.5 模块解析：简单配置好办，复杂逻辑要靠插件

esbuild 内置的解析规则是固定的，**不提供 Webpack 那种"在配置里写解析函数"的入口**。简单的定制可以靠配置项解决：

- `alias`：路径别名（`@/` → `src/`）；
- `external`：把某些包排除在打包之外；
- `resolveExtensions`：调整可省略的扩展名列表；
- `conditions` / `mainFields`：指定用 `exports` 的哪个条件、读 `package.json` 的哪个字段。

真正复杂的解析逻辑（虚拟模块、从网络取模块、按规则改写路径）则要写插件，用 `onResolve` 钩子**完全接管解析**——这也是社区里各种"虚拟模块""~ 前缀路径"方案的标准做法。所以更准确的说法是：**esbuild 不能在插件体系之外定制解析，而不是完全不能定制**。

大多数项目里，`alias` + `external` 已经够用，别没事给自己加戏。
