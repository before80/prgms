...
2020 年 10 月底，esbuild 在 **0.8.1** 中首次提供了**插件系统**（Plugin API）——官方 CHANGELOG 里这一条的原话是 "The initial version of the plugin API (#111)"，最初只有 `onResolve` 和 `onLoad` 两个回调。随后 0.9.x 系列（2021 年 3 月起）继续扩充插件能力，让插件从"两个钩子的雏形"长成一套可用的生态。
...
截至 2026 年 9 月，esbuild 的最新稳定版本为 v0.28.x，依然保持在 0.x 版本区间。
...
v0.8.1 引入插件 API，是 esbuild 生态建设的起点。你可以把它理解为：*"esbuild 从一辆整车变成了一个平台，你可以在上面插各种零件。"*
...
### v0.17.0：上下文 API（增量构建 / watch / serve 三合一）

v0.17.0（2023 年 1 月）是另一个重要节点，引入了全新的 `context` API：先 `createContext()` 拿到一个构建上下文，再用 `ctx.rebuild()` 做增量构建、`ctx.watch()` 开文件监听、`ctx.serve()` 起开发服务器，用完 `ctx.dispose()` 释放资源。

> ⚠️ 顺便纠正一个常见说法：这一版引入的是"上下文 API"，**不是 HMR（热模块替换）**。esbuild 至今没有内置 HMR——`ctx.serve()` 提供的是开发服务器和自动重新构建，浏览器侧的模块热替换仍要交给 Vite 这类上层工具实现（详见第 6 章）。

---

## 2.5 当前稳定版本与发展现状

截至 2026 年 9 月，esbuild 的最新稳定版本为 **v0.28.x**（持续迭代中），版本号随着小版本更新不断攀升。

现状可以用一句话总结：**esbuild 已经从"实验工具"变成了"行业标配"**。

看看这些数据你就知道了：

- Vite 1–7 默认使用 esbuild 作为开发阶段的转译/预构建工具；Vite 8 已改用 Rolldown/Oxc
...
更重要的是，**它们之间不是非此即彼的竞争关系**，而是互相补充。很多工具曾经把 esbuild 内置进来——比如 Vite 1–7 的开发服务器就是用 esbuild 驱动的；Vite 8 之后这条路径由 Rolldown/Oxc 接管。
...
- Vite 1–7 同时用 esbuild 和 Rollup；Vite 8 改为统一的 Rolldown/Oxc
- esbuild 的插件生态越来越丰富
...
我们知道了它诞生于 2020 年，是作者 Evan Wallace 因为受不了 Webpack 的龟速而一气之下写出来的。短短几年时间，它从 0.x 实验版发展到功能完善的 0.28.x，插件系统从无到有，从"大家觉得不实用"到"成为行业标配"。
