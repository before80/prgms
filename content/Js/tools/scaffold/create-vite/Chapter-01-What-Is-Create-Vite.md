...
Create-Vite 的版本号通常与 Vite 主版本保持同步。例如 `create-vite@9.x` 会生成使用 Vite 8.x 的项目。截至 2026 年 9 月，`create-vite` 最新稳定版为 **9.2.x**，生成的模板默认使用 **Vite 8.x**。如果模板中写了 `^8.0.0` 这类依赖范围，具体安装的小版本仍可能略有差异，以生成后的 `package.json` 为准。
...
2020 年，Evan You（尤雨溪）开始发布 Vite 的早期 beta / RC 版本；真正大规模进入生产的稳定版本是 2021 年 2 月发布的 **Vite 2.0**。

Evan You 同时也是 **Vue** 的作者，所以 Vite 和 Vue 的关系天然就很近——Vite 的很多设计理念，都借鉴了 Vue 的简洁哲学。

Vite 的核心理念，用一句话概括就是：

> **开发环境：不打包，直接用原生 ESM；生产环境：用打包器做优化构建。**
...
到了**生产环境**，Vite 才会做完整的优化构建。Vite 1–7 使用 **Rollup**；从 **Vite 8** 开始改用 Rust 编写的 **Rolldown**。Rolldown 兼容 Rollup 插件 API，并继续支持 Tree Shaking、代码分割和更小的产物输出。

```
开发环境：Vite Dev Server → 浏览器原生 ESM → 快如闪电 ⚡
生产环境：Rolldown（Vite 8+；旧版为 Rollup）→ 高度优化的静态文件 → 体积最小化 📦
```
...
- Create-Vite（继承 Vite）的四大核心特性：**极速启动**（不打包，按需返回）、**HMR 热模块替换**（通常数百毫秒内更新，状态保留）、**零配置**（开箱即用，不过度封装）、**原生 ESM**（开发环境信任浏览器，生产环境由 Rolldown 或历史版本的 Rollup 做优化打包）。
