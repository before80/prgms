...
> ⚠️ **关于 CentOS 的重要提醒（2026 年视角）**：很多教材还在拿 CentOS 7/8 举例，但这两个版本都已经**停止维护**了——CentOS Linux 8 于 2021-12-31 结束支持，CentOS Linux 7 于 2024-06-30 结束支持。也就是说，**它们不会再收到任何安全更新**，继续拿来做生产服务器是不合适的（连官方 yum 源都已下线，所以老教程里那些 `yum install` 可能直接连不上）。
>
> 现在的替代路线有三条，都是 RHEL 生态、命令完全一样：
>
> | 选择 | 定位 | 适合场景 |
> |------|------|----------|
> | **Rocky Linux** | 与 RHEL 1:1 兼容的下游重建版 | 想要"老 CentOS 那种稳定"，推荐首选 |
> | **AlmaLinux** | 同样是 RHEL 下游重建版 | 同上，社区活跃度也很好 |
> | **CentOS Stream** | RHEL 的**上游**滚动预览版 | 想紧跟 RHEL 未来版本、做测试环境 |
>
> 本章的命令（`dnf`/`yum`）在 Rocky、Alma、Fedora、RHEL 9/10 上都能照常使用，学一次到处能用。
