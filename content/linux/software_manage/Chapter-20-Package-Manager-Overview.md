...
> ⚠️ **补充（重要）**：CentOS Linux 7 已在 2024-06-30 停止维护，CentOS Linux 8 更早在 2021-12-31 就结束了。**这两个版本都不应再用于生产环境**。新项目请选择 **Rocky Linux**、**AlmaLinux**（RHEL 的下游兼容重建版，最接近老 CentOS 的体验）或 **CentOS Stream**（RHEL 上游滚动版）。它们的包管理命令与本章示例完全一致。
>
> 另外，在 RHEL 8+ / Rocky 8+ 上，`yum` 只是一个指向 `dnf` 的软链接（历史上 RHEL 8 提供 `yum` 兼容命令，RHEL 9 更是把 `yum` 变成了 `dnf` 的别名）。**新脚本建议统一写 `dnf`**。
