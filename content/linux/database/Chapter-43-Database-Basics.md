...
| SQL标准支持 | 几乎完美支持 | 部分支持 |
| 事务支持 | 完整ACID，且**DDL也支持事务**（建表失败可回滚） | InnoDB 引擎下同样是完整 ACID，但 DDL 不支持回滚 |
| 性能 | 复杂查询更强 | 简单查询更快 |
| 扩展性 | 自定义类型、函数 | 插件系统 |
...
**Oracle的"钞能力"代价：**
```
Oracle标准版2（SE2）：约 17,500 美元/处理器
Oracle企业版（EE）：   约 47,500 美元/处理器
```

对，你没看错，是**每处理器（per processor）**，不是每用户，而且这只是**软件许可的报价（List Price）**，
另外还要按许可价的大约 22% 每年交服务费。Oracle 的"处理器"按核心数折算（一般 1 个处理器 = 2 个核心），
所以一台 32 核的机器，光是企业版许可就要按 16 个处理器来算……

> 这些价格是公开报价，实际采购会有折扣，也会随时间调整。这里列出来只是让你有个"量级"的概念，
> 别把它当合同价格。
...
graph TB
    ...
    A --> D[宽列数据库<br/>Wide-Column Store]
...
| 软肋 | 说明 | 解决方案 |
|------|------|----------|
| **查询语言各不相同** | 得学新的查询语言/API | 各有各的客户端；不少产品也提供了类SQL（如 Cassandra 的 CQL） |
| **事务能力较弱** | 早期大多不支持多行事务 | 现在 MongoDB 4.0+、Redis、DynamoDB 都提供了有限的事务支持 |
...
- **性能优秀**：默认存储引擎是 WiredTiger，配合内存缓存，随机读写都很快
  （顺带一提：早期版本的 mmapv1 存储引擎已在 MongoDB 4.2 被彻底移除，老教程里"基于内存映射文件"的说法已经过时）
...
### 43.3.3 Cassandra——宽列数据库的性能怪兽

**Cassandra** 是Facebook开源的宽列（wide-column）分布式数据库，现在属于Apache基金会。

> ⚠️ 先澄清一个特别容易混的概念：**"宽列数据库"和"列式数据库"不是一回事！**
>
> - **宽列数据库（Wide-Column Store）**：Cassandra、HBase 属于这一派。数据模型是
>   "行键 → 多个列族 → 动态列"，看起来列很多，但**底层依然是按行（分区）存储的**，
>   擅长按行键高并发读写，**不擅长做聚合分析**。
> - **列式数据库（Columnar / OLAP）**：ClickHouse、Doris、Parquet 这一类才是"按列存"，
>   同一列的数据连续存放，压缩率高，做 `SUM`、`COUNT` 这类聚合特别快。
>
> 所以"Cassandra 是列式数据库"是流传很广的错误说法，学的时候一定要分清。

| 类型 | 代表 | 存储方式 | 优势 | 劣势 |
|------|------|----------|------|------|
| **行式存储** | MySQL、Oracle、PostgreSQL | 一行的数据连续存放 | 按行读写快、事务强 | 只查几列也要读整行，聚合分析慢 |
| **列式存储（OLAP）** | ClickHouse、Doris、Parquet | 同一列的数据连续存放 | 聚合分析快、压缩率高 | 逐行点查不划算、更新代价高 |
| **宽列存储** | Cassandra、HBase | 按行键分区，列族可动态扩展 | 海量数据、高并发写入、易扩展 | 查询能力受限（基本只能按行键/索引查） |
...
- **四大流派**：
  ...
  - **宽列数据库（Cassandra）**：海量数据、高并发写入，无单点故障
  - **列式数据库（ClickHouse）**：面向分析的 OLAP，聚合查询飞快
