...
> 📌 **本章定位**：这一章是"Web 服务"这条线里的数据库入门——**为什么要在这里讲数据库？** 因为装完 Nginx、跑起 LAMP/LEMP 之后，你的网站马上就会需要存数据：用户、文章、订单，全都得落库。本章帮你把"数据库是什么、怎么建库建表、怎么写最基本的 SQL"这一步走通。
>
> 更深入的数据库内容（MySQL/MariaDB 运维、PostgreSQL、Redis 缓存、消息队列）在《数据库》卷里，具体是第 44~47 章。**如果你已经熟悉 SQL 基础，可以跳过本章直接去那边。**

## 43.0 一个网站的数据是怎么流动的

先把 Web 与数据库的关系用一张图理顺，后面看 SQL 语句才不会觉得"跟网站没关系"：

```mermaid
flowchart LR
    U["浏览器"] -->|"HTTP 请求"| W["Web 服务器<br/>Nginx / Apache"]
    W -->|"转发动态请求"| A["应用服务器<br/>PHP / Python / Java / Node"]
    A -->|"SQL 查询"| D[("数据库<br/>MySQL / PostgreSQL")]
    D -->|"结果集"| A
    A -->|"HTML / JSON"| W
    W -->|"HTTP 响应"| U
    A -.->|"热点数据先读缓存"| C[("Redis 缓存")]
```

这张图里有三个关键点，值得先记住：

1. **Web 服务器不直接查数据库**。真正执行 SQL 的是应用代码（PHP/Java/Python…），Nginx 只负责把请求转过去。
2. **数据库往往是整个网站的瓶颈**。因为它的状态最难扩展，所以后面才需要缓存、读写分离、分库分表这些手段。
3. **"能跑"和"安全"是两件事**。数据库账号、连接方式、备份策略，决定了你的网站是"能演示"还是"能上线"——这也是本章和下一卷反复强调的重点。
