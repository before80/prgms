...
接下来登场的是**系统用户**，UID范围是**1到999**（上限取决于发行版的 `SYS_UID_MAX` 配置：老的 RHEL/CentOS 6 是 1-499，Debian/Ubuntu 和 RHEL 7 以后都是 1-999）。
...
看到这些名字了吧？`daemon`是守护进程（daemon这个名字来源于希腊神话中的"daimon"，意思是"超自然的存在"），`www-data`是给网页服务器打工的。

> **`nobody` 是个例外**：它的 UID 是固定的 **65534**（不在 1-999 里），所以上面那条 awk 命令**不会**把它筛出来。`nobody` 是"谁都不属于"的幽灵用户，常用来运行不需要任何特权、又怕被滥用的进程（比如某些网络的匿名访问）。类似的还有 `nogroup`（GID 65534）。
...
```mermaid
pie title Linux 用户分类（数量级示意，非绝对数值）
    "root (UID 0)：超级管理员" : 1
    "系统用户 (UID 1-999)：守护进程" : 200
    "普通用户 (UID 1000+)：真人用户" : 50
```
...
> [!NOTE]
> 为什么GID和UID都是1000？**不是巧合，而是默认行为**：`useradd` 默认会为新用户创建一个同名主组（这一行为由 `/etc/login.defs` 里的 `USERGROUPS_ENAB yes` 控制），而 GID 和 UID 的最小值、增长方式基本一致，所以新建用户时两者往往相同。你完全可以手动把两者设成不同的值。
...
- UID 1000以下是系统用户
- UID 1000-60000是普通用户
- 60000-65533：`useradd` 默认**不会**自动分配这一区间，通常留给手动指定的特殊用途（或某些 NFS/容器映射场景）

> **Debian/Ubuntu 用户请注意**：这两个发行版还额外提供了一个更友好的 `adduser` 命令（注意比 `useradd` 多一个 d）——它会交互式地提示设置密码、自动创建家目录、自动加上 `-m -s`，适合手工建账号；而 `useradd` 是底层命令，适合脚本。可以先用 `adduser xiaoming` 体验一下它和 `useradd` 的区别。
...
> **想把一个人彻底关在门外**，光锁密码还不够（他的 SSH 密钥依然能进来）。彻底禁用的组合拳是：
>
> ```bash
> sudo usermod -L 用户名                      # 锁密码
> sudo usermod -s /usr/sbin/nologin 用户名    # 把登录 Shell 换成 nologin
> sudo chage -E 0 用户名                      # 让账户立即过期（0 表示从1970年起就过期）
> ```
>
> 只锁密码（`passwd -l`）是一种"软锁定"，适合临时停用；三种一起上才是真禁用。
...
# 格式：用户名 密码状态 最后修改日期 最小天数 最大天数 警告期 失效期(inactive)
# 密码状态：P=有密码，L=锁定，NP=无密码
```

> **最后那个 `-1` 不是"过期"**：它是 `inactive`（密码过期后账户被彻底禁用之前的宽限天数），`-1` 表示"不设置，永不因密码过期而禁用账户"。别把它和"账户过期日期"搞混——那个要用 `chage -E` 设置、`chage -l` 查看。
...
### 15.8.1 格式：用户名:加密密码:最后修改:最小:最大:警告:失效:保留:账户过期
...
| 7 | 失效天数（inactive） | 密码已过期，但账户再宽限多少天才会被禁用 | 空 |
| 8 | 保留字段 | 预留的，目前没用 | 空 |
| 9 | 账户过期日 | 账户到这一天就作废（从1970-01-01算起的天数），空表示永不过期 | 空 |

> **第 7 和第 9 个字段最容易记混**：第 7 个是"密码过期后仍然放过你几天"，第 9 个是"这个账号本身到哪天作废"（相当于 `chage -E`）。两个都空着表示"不管"。
...
# *        : 密码被设置为不可用
# 加密密码的前缀告诉你用了哪种算法：
#   $1$       MD5（非常老，已不安全，千万别再用）
#   $5$       SHA-256
#   $6$       SHA-512（很长时间的历史默认值，比 yescrypt 更易被暴力破解）
#   $2y$/$2b$ bcrypt（基于 Blowfish，抗暴力破解好）
#   $y$       yescrypt（现代发行版的默认，如 Debian 12 / Ubuntu 22.04+）
```

> **为什么现在不用 MD5/SHA 而用 yescrypt？** MD5、SHA-256、SHA-512 都是"快"哈希，显卡每秒能算几十亿次，适合拿字典硬碰。yescrypt、bcrypt 是"慢"哈希，还能按需调高计算成本，破解成本高得多。查看自己的系统默认用什么算法：`grep ^ENCRYPT_METHOD /etc/login.defs`（现代系统一般是 `YESCRYPT`）。
...
> [!IMPORTANT]
> `/etc/shadow` 文件包含加密的密码信息，**绝对不能让普通用户读取**！如果普通用户能读 shadow 文件，就可以把哈希拿去离线爆破。所以这个文件权限非常严格——不同发行版的默认值不完全一样：
> 
> ```bash
> ls -l /etc/shadow
> # Debian/Ubuntu：-rw-r----- 1 root shadow  ... /etc/shadow   （640，shadow 组可读）
> # RHEL 系：     ---------- 1 root root    ... /etc/shadow   （000，只有 root 能碰）
> ```
>
> **别照抄"000 才是对的"**：Debian 系给 `shadow` 组留了读权限是为了让 `chage`、`passwd -S` 这类工具正常工作，是设计如此。判断标准只有一条：**普通用户的 UID 不能读到它**。可以用 `sudo -u 某个普通用户 cat /etc/shadow` 测一下（预期报 `Permission denied`）。
...
4. **重要配置文件**：
   ...
   - 查这些信息优先用 `getent passwd 用户名` / `getent group 组名`，它会按系统真正的解析顺序（含 LDAP/SSSD）查，比直接 `grep /etc/passwd` 更可靠
...
6. **安全提醒**：
   - 锁定用户光靠 `passwd -l` 不够，SSH 密钥仍可登录；要配合 `nologin` + `chage -E 0`
   - 删除用户用 `userdel -r` 会连家目录一起删，动手前务必确认或先备份
