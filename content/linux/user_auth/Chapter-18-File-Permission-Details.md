...
# 符号法要达到同样的效果，需要一步步写清楚：
chmod u=rwx,g=rx,o=rx file

# 注意：chmod u+x,go-wx file 并不等价于 755！
# 它只是在原有权限上"给所有者加 x、给组和其他人去掉 w 和 x"，
# 最终结果取决于原来的权限。比如原权限是 644 时，执行完是 744。
...
> **符号法 vs 数字法**：数字法（`755`）是"**直接设定**成这个值"，符号法（`u+x`）是"在**现有权限基础上**加减"。所以"只加不减"的场合用符号法更方便（不用先搞清楚当前是什么权限），要精确得到某个固定值就用数字法。
...
**umask** 是一个"权限掩码"：**掩码里为 1 的位，就是要从新文件权限里"抹掉"的位**。

> **别再用"减法"了！** 很多教程写"新权限 = 最大权限 - umask"，在 umask 是 022 时碰巧算对（6-0=6、6-2=4、6-2=4），换成 umask 027 就完全错了——`666` 减 `027` 根本得不到 `640`。真实规则是按位与：`新权限 = 最大权限 AND (NOT umask)`。你不需要背公式，只要记住：**umask 里写着几，就减掉哪一类权限**（用权值 r=4、w=2、x=1 自己去掉）。

### 18.9.1 文件默认：666 去掉 umask 的位

```bash
# 文件的最大权限是 666（rw-rw-rw-）——注意不含 x，普通文件默认不给执行权限
# 再"抹掉"umask 中为 1 的位，就是最终权限

# umask = 022 表示"抹掉 组 和 其他人 的写权限（w = 2）"
# 从 rw-rw-rw- 去掉这两类 w，得到
# rw-r--r--  →  644
```

### 18.9.2 目录默认：777 去掉 umask 的位

```bash
# 目录的最大权限是 777（rwxrwxrwx）——目录默认带 x，否则进不去
# 再抹掉 umask 中为 1 的位

# umask = 022：抹掉组和其他人的 w
# 从 rwxrwxrwx 去掉这两类 w，得到
# rwxr-xr-x  →  755
```
...
```bash
# umask 的 4 位含义（从右往左看更直观）：
#   最后一位 2：其他人的权限里要去掉 w
#   倒数第二位 2：所属组的权限里要去掉 w
#   倒数第三位 0：所有者的权限一位都不去掉
#   最左边那位 0：特殊权限位（SUID/SGID/Sticky Bit）不去掉
# 所以 0022 = "组和其他人都不给写权限"，这是最常见的默认值
```
...
# umask 027 的效果：去掉 组的 w(2)，以及 其他人的 rwx(7)
# 文件：rw-rw-rw- 去掉组 w、其他人 rwx → rw-r-----（640）
# 目录：rwxrwxrwx 去掉组 w、其他人 rwx → rwxr-x---（750）
```
...
```mermaid
graph LR
    A["文件最大权限 666"] --> B["抹掉 umask 中为 1 的位<br/>（umask 022 → 去掉组和其他人的 w）"]
    C["目录最大权限 777"] --> B
    B --> D["新文件权限 = 644<br/>rw-r--r--"]
    B --> E["新目录权限 = 755<br/>rwxr-xr-x"]
...
> **为什么文件"最大权限"是 666 而不是 777？** 因为新建普通文件默认**不会**带上执行权限——这是安全设计：从网上下载一个陌生文件，它不应该是可执行程序。目录则必须带 x（否则进不去），所以目录的最大权限是 777。想给文件加执行权限，得手动 `chmod +x`。
...
前面一直在用 `ls -l` 看权限，但那一长串字符到底每一项是什么意思？拿一个真实输出拆开看：

```
-rw-r--r--  1  longx  developers  4096  Mar 23 12:00  report.txt
drwxr-xr-x  2  longx  developers  4096  Mar 23 12:00  mydir
lrwxrwxrwx  1  root   root          10  Mar 23 12:00  link -> /etc/hosts
```

| 位置 | 示例 | 含义 |
|------|------|------|
| 第 1 个字符 | `-` / `d` / `l` | 文件类型：`-`普通文件、`d`目录、`l`符号链接、`b`块设备、`c`字符设备、`s`套接字、`p`管道 |
| 第 2-4 个字符 | `rw-` | **所有者**的权限 |
| 第 5-7 个字符 | `r--` | **所属组**的权限 |
| 第 8-10 个字符 | `r--` | **其他人**的权限 |
| 紧随其后的数字 | `1` | **硬链接数**（目录的这个数字表示它有多少个子目录，至少为 2：`.` 和 `..`） |
| 第一个名字 | `longx` | 文件**所有者** |
| 第二个名字 | `developers` | 文件**所属组** |
| 数字 | `4096` | 文件大小（字节）；目录显示的是目录项占用的空间（通常是 4096，不代表里面文件的大小） |
| 日期 | `Mar 23 12:00` | 最后修改时间（超过半年的文件会显示年份而不是时间） |
| 最后 | `report.txt` | 文件名；符号链接会多显示 `-> 目标路径` |

> **三个特殊字符也会出现在这 10 位里**：如果看到 `s` 或 `t`，说明该文件带 SUID/SGID/Sticky 特殊权限（下一章细讲）。例如 `-rwsr-xr-x` 里的 `s` 就是 SUID，`drwxrwxrwt`（如 `/tmp`）末尾的 `t` 就是 Sticky Bit。
...
# 原因：sudoers 文件权限被改坏了（sudo 会拒绝加载"任何人都能改"的配置，
#      因为它意味着攻击者可以偷偷把你变成 root）

# 解决：
# 1) 如果你还有另一个 root 会话/单用户模式，直接修权限：
sudo chmod 0440 /etc/sudoers
sudo chown root:root /etc/sudoers

# 2) 如果已经完全进不去 sudo（被自己锁住了）：
#    - 用 su - 切到 root（需要知道 root 密码），或
#    - 重启进入单用户/救援模式（recovery mode）后修改
#    - 云服务器可用 VNC/控制台
```

> **如果连 `/etc/sudoers.d/` 下的某个文件权限不对**（比如 644、666），症状类似：`sudo: /etc/sudoers.d/zhangsan is world-writable`。修复方式是 `sudo chmod 0440 /etc/sudoers.d/文件名`。**记住这条铁律：sudoers 主文件和 sudoers.d 下的文件都必须是 `0440` 且属主属组为 root**。
...
graph TD
    ...
    H["umask"] --> I["新权限 = 最大权限 抹掉 umask 中为 1 的位"]
...
5. **umask**：
   - 文件默认666，目录默认777
   - 算法是"从最大权限里抹掉 umask 标记为 1 的位"，不是做减法
