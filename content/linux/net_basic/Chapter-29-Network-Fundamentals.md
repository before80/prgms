...
这是网络的"公路"，负责把数据的"0和1"转换成电信号、光信号或无线电波。网线、光纤、网卡、中继器、集线器（Hub）都在这层干活。**注意交换机属于数据链路层**——它要看 MAC 地址来决定把帧从哪个口发出去，那是下一层的活儿。
...
graph TB
    ...
    C["② 网络层<br/>Internet Layer<br/>IP/ICMP"]
    D["① 网络接口层<br/>Network Interface Layer<br/>Ethernet/PPP/Wi-Fi/ARP"]
...
| 传输层 | 传输层 | TCP、UDP |
| 网络层 | 网络层 | IP、ICMP、路由协议（OSPF/BGP） |
| 数据链路层、物理层 | 网络接口层 | Ethernet、PPP、Wi-Fi、ARP |

TCP/IP四层模型更实用，因为它就是Internet实际运行的标准。很多资料把这四层说成五层（把网络接口层拆成物理层+数据链路层），这是为了教学方便，不影响理解。

> **ARP 到底算哪一层？** 它夹在二三层之间：靠广播在局域网里问"这个 IP 是谁的 MAC"，所以放在**网络接口层（链路层）**；但因为它处理的是 IP 地址，也有资料把它归到网络层。上面按"它干的活属于链路层"来归类，考试和面试时按对方教材的口径答即可。
...
NAT（Network Address Translation，网络地址转换）技术就是用来把私有IP"翻译"成公有IP访问互联网的。最常见的两种：

- **SNAT（源地址转换）**：改的是"发出去的包的源地址"，让内网设备共用一个公网 IP 上网——家庭路由器做的就是这个（更准确地说，是靠端口区分不同内网连接的 **NAPT / 端口复用**，一台路由器能让几十台设备共用一个公网 IP）
- **DNAT（目标地址转换）**：改的是"进来的包的目标地址"，把公网请求转发给内网某台机器——端口映射（Port Forwarding）就是它

> **NAT 带来的副作用**：外网**主动**连不进来（没有端口映射时），内网设备之间也无法直接被外部访问——这就是常说的"NAT 后面的机器没有公网 IP"。很多 P2P、视频通话、游戏联机需要"打洞"（NAT 穿透）技术来绕开这个问题。
...
除了私有地址，还有一批"一看就要反应过来"的特殊地址，故障排查时会反复遇到：

| 地址 | 含义 | 用途 |
|------|------|------|
| `0.0.0.0` | 未指定地址 | 服务端监听时表示"监听本机所有网卡"；路由表里表示"默认路由" |
| `127.0.0.1` | 环回地址（本机） | 只能本机访问自己，也叫 `localhost`；整个 `127.0.0.0/8` 都是环回 |
| `255.255.255.255` | 受限广播地址 | 只在本地链路广播，路由器不会转发 |
| `169.254.x.x` | 链路本地地址（APIPA） | **没能从 DHCP 拿到 IP 时系统自动配的**，看到它基本说明"DHCP 没成功" |
| `100.64.0.0/10` | 运营商级 NAT（CGNAT） | 运营商给用户用的"大内网"地址，本质仍是私有地址 |
| `224.0.0.0/4` | 多播地址 | 一对多通信，如 OSPF 用的 `224.0.0.5` |

> **排障口诀**：看到 `169.254.x.x`，先查网线/Wi-Fi 和 DHCP；看到 `127.0.0.1`，说明只在本机；看到 `0.0.0.0`，说明在监听全网卡或表示默认路由。
...
**动手算一次子网**（很多教程只给结论，其实自己算一遍就懂了）。以 `192.168.1.100/26` 为例：

1. `/26` 表示前 26 位是网络位，主机位只有 `32 - 26 = 6` 位
2. 每个子网大小 = $2^6 = 64$ 个地址，所以子网边界是 0、64、128、192
3. `192.168.1.100` 落在 `64~127` 这一段
4. 于是：网络地址 = `192.168.1.64`，广播地址 = `192.168.1.127`，可用主机范围 = `192.168.1.65 ~ 192.168.1.126`，可用主机数 = $2^6-2 = 62$ 台

> **记住两个"全"**：主机位**全 0** 是网络地址，主机位**全 1** 是广播地址，这两个都不能分配给主机，所以要 `-2`。唯一的例外是 `/31`（点对点链路用两个地址）和 `/32`（单主机），现代设备上不再减 2。
...
sequenceDiagram
    ...
    A->>S: ① ARP 请求（广播帧）<br/>"谁是 192.168.1.200？告诉 192.168.1.100"
    S->>B: ② 交换机把广播复制到所有端口
    B->>S: ③ ARP 应答（单播帧）<br/>"我是，MAC 是 aa:bb:cc:dd:ee:ff"
    S->>A: ④ 交换机按目标 MAC 送回发送方
    Note over A,B: 此后 A 把 IP-MAC 存入 ARP 缓存，数据帧直接发往该 MAC
```

> **交换机在这里只是"转发者"**：真正回答 ARP 问题的是目标主机，不是交换机。交换机负责把广播帧复制给所有端口（②），再根据应答帧里的目标 MAC 单播送回（④）。
...
? (192.168.1.200) at 00:0c:29:5a:6b:7c [ether] on eth0
```

> **新系统推荐用 `ip neigh`**：`arp` 命令来自老旧的 net-tools，很多现代发行版默认不再安装。等效的现代命令是 `ip neigh`，它还能显示条目状态（REACHABLE / STALE / FAILED）：
...
> **缓存多久清一次？** Linux 的邻居表默认大约几十秒（`REACHABLE` 约 30 秒左右）就会转成 `STALE`，需要时再重新确认——**不是**有些资料说的"15 到 20 分钟"。可以手动清理某一条：`sudo ip neigh del 192.168.1.200 dev eth0`。
...
> **注意区分**：像 `3306`（MySQL）、`3389`（RDP）、`5432`（PostgreSQL）、`6379`（Redis）这些我们天天见的端口，其实**不在 0-1023 里，而属于下一节的"注册端口"**。不要因为它们常见就以为是知名端口。
> 另外，"只有 root 能绑定 <1024 端口"是 Linux 的默认限制，管理员也可以通过 `CAP_NET_BIND_SERVICE` 能力或 `net.ipv4.ip_unprivileged_port_start` 内核参数放开给普通程序。

### 29.12.2 注册端口：1024-49151

这些端口由IANA注册给特定的应用服务。

例如：1080（SOCKS代理）、1433（MS SQL Server）、1521（Oracle）、3306（MySQL）、3389（RDP）、5432（PostgreSQL）、6379（Redis）、8080（HTTP代理/备用Web端口）、8443（HTTPS备用端口）。

> **注册端口只是"登记"，不保证独占**：IANA 登记某个端口给某服务，并不等于别人不能用。所以换一台机器发现 8080 被"别的程序"占了，是完全正常的。
...
> **IANA 的 49152-65535 只是"建议范围"，操作系统未必照抄**：Linux 默认用的是 `32768-60999`（可用 `sysctl net.ipv4.ip_local_port_range` 查看或修改），Windows 才用 `49152-65535`。所以看到 Linux 上出现 `49234`（在建议范围内）或 `3xxxx`（在 Linux 实际范围内）当源端口，都是正常的。
...
graph LR
    ...
    W --> P80["80 · HTTP"]
    W --> P443["443 · HTTPS"]
    W --> P8080["8080 · 备用 Web / 代理"]
    M --> P22["22 · SSH"]
    M --> P23["23 · Telnet（明文，已淘汰）"]
    M --> P3389["3389 · RDP"]
    D --> P3306["3306 · MySQL"]
    D --> P5432["5432 · PostgreSQL"]
    D --> P6379["6379 · Redis"]
    O --> P53["53 · DNS"]
    O --> P25["25 · SMTP"]
    style R fill:#ffffcc
...
> **记忆技巧**：HTTP 是 80、HTTPS 是 443（加了 S 就换了号）、SSH 是 22、Telnet 是 23 且已被 SSH 取代、DNS 是 53。MySQL（3306）、Redis（6379）这类"四位数端口"记住就好，别指望找规律。
...
- **NAT**：SNAT 让内网共用公网 IP 出去，DNAT 让外部请求转进内网；看到 `169.254.x.x` 就是没拿到 IP。
