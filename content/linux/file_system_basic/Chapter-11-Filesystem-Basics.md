...
2008年，ext4 正式发布！它是目前**使用最广泛**的 Linux 文件系统之一，也是 Ubuntu、Debian 等发行版的默认选择。

```bash
# ext4 文件系统的特点：
# - 最大文件大小：16TB
# - 最大文件系统大小：1EB（艾字节！1EB = 100 万 TB）
# - 支持日志
...
> 📌 **别搞混"谁是谁的默认"**：文件系统的默认选择**取决于发行版**，没有"唯一标准答案"——
>
> | 发行版 | 默认文件系统 |
> |--------|--------------|
> | Ubuntu / Debian | ext4 |
> | RHEL / Rocky / Alma / Fedora（服务器） | XFS |
> | Fedora 工作站 | Btrfs |
> | openSUSE | Btrfs |
>
> 所以你不会在 RHEL 上看到"默认 ext4"，也不会在 Ubuntu 上看到"默认 XFS"。**选哪个主要看你的需求**：通用场景 ext4 足够省心，超大文件/高并发用 XFS，想要快照和校验就用 Btrfs/ZFS。
