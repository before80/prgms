...
```mermaid
graph LR
    A["互联网<br>进来的数据包"] -->|"逐条对照规则"| B["防火墙<br>Firewall"]
    B -->|"规则允许，放行"| C["服务器"]
    B -->|"规则拒绝，丢弃"| D["直接扔掉，不回任何响应"]
    C -->|"服务器发出的响应"| B
    B -->|"状态表里查得到，放行"| A
    style B fill:#ffcccc
    style C fill:#ccffcc
...
> 说明：状态检测防火墙的核心在于那张"状态表"——只要某个连接的第一包被放行，后续属于该连接的包都会被自动放行，不必为每个包重复写规则。
...
```mermaid
graph LR
    subgraph "应用层防火墙能看到什么"
        A["一个 HTTP 请求"] --> B["请求方法 + URL<br>GET /admin/delete"]
        B --> C["请求头<br>Cookie、User-Agent、X-Forwarded-For"]
        C --> D["请求体<br>表单或 JSON 数据"]
        D --> E{"判断：像正常业务还是攻击？"}
        E -->|"正常"| F["放行"]
        E -->|"像 SQL 注入 / XSS"| G["拦截并记录"]
    end
    style A fill:#ccffcc
    style F fill:#ccffcc
    style G fill:#ffcccc
...
包过滤防火墙到"请求头"这一层就看不进去了，而应用层防火墙能把整个 HTTP 请求拆开逐段检查——这就是 WAF（Web 应用防火墙）能拦住 SQL 注入、XSS 的原因。
...
| 端口 | 服务 | 说明 | 建议 |
|------|------|------|------|
| 22 | SSH | 远程管理 | 仅对管理 IP 开放，最好只允许密钥登录 |
| 80 | HTTP | Web 服务 | 开放（建议 301 跳转到 HTTPS） |
| 443 | HTTPS | 加密 Web | 开放 |
| 20 / 21 | FTP | 数据连接 / 控制连接 | **建议关闭**，用 SFTP（走 SSH 的 22 端口）替代 |
| 23 | Telnet | 明文远程登录 | **必须关闭** |
| 25 / 587 | SMTP | 邮件投递 / 客户端提交 | 只有邮件服务器才需要开放 |
| 53 | DNS | 域名解析 | TCP 和 UDP 都要（大响应走 TCP）；**只对需要它的客户端开放**，对全网开放等于给 DNS 放大攻击当帮凶 |
| 110 / 995 | POP3 | 收邮件（明文 / TLS） | 只有邮件服务器需要 |
| 143 / 993 | IMAP | 收邮件（明文 / TLS） | 只有邮件服务器需要 |
| 3306 | MySQL | 数据库 | **禁止公网访问**，只监听 127.0.0.1 或内网 |
| 5432 | PostgreSQL | 数据库 | **禁止公网访问** |
| 6379 | Redis | 缓存数据库 | **禁止公网访问**；Redis 被入侵绝大多数是因为暴露了它 |
| 27017 | MongoDB | NoSQL 数据库 | **禁止公网访问** |
| 3389 | RDP | Windows 远程桌面 | 仅对管理 IP 开放，别直接暴露公网（勒索软件的首选入口） |
| 445 | SMB | Windows 文件共享 | 建议关闭；历史上多次被蠕虫大规模利用 |
| 8080 / 8443 | 备用 Web 端口 | 应用自定 | 按需开放，不要为了省事长期开着 |

> **安全配置原则**：防火墙规则越少越好，权限越小越安全。Web服务器只需要开放80/443，数据库只需要本地访问，SSH只对管理员IP开放。

一个实用的判断方法：**先问"这个端口需要被谁访问"**——

- 只被本机访问（数据库、Redis）：监听 `127.0.0.1`，防火墙里一条规则都不用加；
- 只被内网访问（MySQL、内部 API）：放行内网网段，绝不对 `0.0.0.0/0` 开放；
- 必须对公网开放（80、443）：开放最小集合，其余一律拒绝。
...
- **主机防火墙 ≠ 云安全组**。云服务器的安全组在宿主机/虚拟网络层生效，系统里的 UFW/firewalld 在操作系统里生效，**两层都要配**；安全组放行了、系统防火墙没放行，服务照样访问不了；
- **防火墙不是万能的**。它只管"能不能连上"，管不了"连上之后你做了什么"。弱密码、过期的组件、被上传的 webshell，都是防火墙挡不住的，安全是分层的事。

下一章，我们开始在 Ubuntu 上用 UFW 配置防火墙。
