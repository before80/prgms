...
在 VS Code 里跑 Notebook，要选的不是 Python 解释器而是**内核**：右上角切换内核时，实际选的是某个环境里的 `ipykernel`。所以"能不能跑"取决于当前环境有没有装 `ipykernel`。
...
Watch 面板里可以填变量名，也可以填**任意表达式**：写 `total / len(numbers)` 这样的式子，VS Code 会在每次暂停时重新求值，比在调试控制台里反复敲命令省事得多。
