...
于是，他们开始了漫长的探索。1991 年，Gosling 写出了这门语言的第一个版本，取名叫 **Oak**（橡树）。团队还做了一个叫 **\*7**（读作 "Star Seven"）的掌上设备原型来演示它——请注意，\*7 是那台**设备**的名字，不是语言的名字，很多资料会把这两者写混。1995 年，Oak 正式改名为 Java 对外发布。
...
> ⚠️ **必须知道的重要更新**：`SecurityManager`（安全管理器）从 Java 17 起就被标记为"废弃并计划移除"，**Java 24（JEP 486）已经把它永久禁用**——现在调用 `System.setSecurityManager()` 会直接抛出 `UnsupportedOperationException`，而 `System.getSecurityManager()` 永远返回 `null`。与此同时，Java Applet 插件在 Java 9 被标记废弃、Java 11 从 JDK 中移除，主流浏览器从 2017 年起也不再支持。所以下面这段代码只是一段**历史概念演示**，在现代 JDK 上它永远会走 `else` 分支；请把它当作"Java 早年怎么实现安全性"来看，不要以为它今天还能用来做权限控制。
```java
/**
 * 演示 Java 的安全管理器概念
...
public class MultiThreadDemo {
    public static void main(String[] args) {
        ...
        // 创建线程：给 Thread 传入一个 Runnable 任务（这里用 Lambda 简写）
...
public class NetworkDemo {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 把字符串解析成 InetAddress：传 IP 字面量或域名都可以
...
- **JRE**：JVM + 运行时类库 = 跑 Java 程序所需的全部东西（注意：`rt.jar` 这种“一个大 jar 装下整个类库”的形式，在 Java 9 引入模块化后就取消了，现在类库以 `java.base` 等模块的形式提供）
...
> **实战建议：** 作为 Java 开发者，你一定需要安装 **JDK**。如果你只是运行别人写好的 Java 程序（很少见），理论上装个 JRE 就够了——不过要提醒一句：从 Java 11 开始 Oracle 已经不再单独发布 JRE 安装包了（需要的话可以用 `jlink` 自己裁剪一个运行时镜像），所以今天绝大多数人直接装 JDK 就行。
...
根据各家机构的估算，全球约有 **800 万到 1200 万** Java 开发者（不同统计口径差异较大，这个数字仅供参考）。
...
| 对比项 | Java | Python |
|--------|------|--------|
| **设计哲学** | 静态强类型、编译成字节码、面向对象 | 动态强类型、解释执行、多范式 |
...
| **内存管理** | GC（自动，追求高吞吐） | GC（自动，追求低延迟，暂停时间更短） |
| **学习曲线** | 中等 | 平缓 |
| **编译速度** | 较慢 | 极快 |
...
| **async/await** | 没有 async/await 关键字，异步靠 `CompletableFuture`、虚拟线程等实现 | 语言层面原生支持 async/await |
...
- **健壮性**：强类型检查 + 异常处理机制
- **安全性**：字节码校验 + 沙箱安全模型（提醒：`SecurityManager` 已在 Java 24 被永久禁用、Applet 也早已移除，这两项如今主要是历史概念）
- **多线程**：内置多线程支持
- **高性能**：JIT 编译器运行时优化
- **分布式**：原生支持网络编程
...
| 语言 | vs Java 的优势 | 劣势 |
|------|---------------|------|
| Python | 开发速度快、AI/ML 生态强 | 运行较慢，大型企业级系统的工程积累较少 |
| C++ | 性能更强、底层可控性更高 | 学习难度大，需要为每个平台分别编译 |
| JavaScript | 浏览器端事实标准、全栈能力 | 动态类型，大型项目需要额外的工程约束 |
| Go | 并发模型简单、云原生生态好 | 语言特性和第三方库生态相对精简 |
| C# | 语言特性新、.NET 生态成熟 | 跨平台生态的覆盖面不如 JVM 广 |
...
> **预告：** 下一章我们先沿着时间线，看清 Java 是怎么从 1.0 一路走到今天的——每个版本都带来了什么，为什么招聘要求里会出现"Java 8""Java 11""Java 17""Java 21"这些数字，以及它们各自意味着什么。
