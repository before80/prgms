...
> 💡 **栈 vs 堆**：Java 内存分为栈（Stack）和堆（Heap）。`scores` 变量本身存在栈中，它是一个"地址指针"，指向堆里的实际数组对象。**整个数组对象（包括 int 等基本类型的元素值）都存储在堆内存中**——这一点很重要，基本类型的数组不是"值存在栈"，而是整个数组对象都在堆里。
...
// ❌ 下面的写法也是错的——初始化列表不能脱离声明单独使用
int[] a;
a = {1, 2, 3};        // 编译错误！需要写成 a = new int[]{1, 2, 3};
```

> 💡 **准确说法**：`{1, 2, 3}` 这种"初始化列表"只能**直接写在声明语句里**（写不下时换行是允许的，只要它还是声明的一部分）。一旦拆成"先声明、后赋值"两句，右边就必须写完整的 `new int[]{1, 2, 3}`。
...
下面这段代码和第 11.2.5 节很像，只是多了一行赋值。先别急着往下看，自己猜猜两行输出分别是什么：
...
把前面学的遍历、累加、比较串起来，写一个真正有用的小程序——一次遍历同时求出总分、平均分、最高分和最低分：
...
二维数组在内存里是「数组的数组」：外层数组的每个元素，都是指向一个一维数组的引用。请看下图：
...
遍历二维数组就是「套两层循环」：外层走行，内层走列。用普通 for 循环时，内层循环的边界要写成 `grades[row].length`，这样才能兼容上一节的不规则数组：
...
矩阵相乘是二维数组最经典的应用场景：结果矩阵 C 的第 i 行第 j 列，等于 A 的第 i 行和 B 的第 j 列逐项相乘再求和。下面用三重循环实现它：
...
一维数组用 `toString()`，多维数组就要用 `deepToString()`，否则打印出来只是一串看不出内容的地址：
...
> 💡 **左闭右开**：`Arrays.sort(arr, 0, 3)` 排序索引 `[0, 3)`，也就是只排序 0、1、2 三个位置，不包含索引 3。Java 标准库里的范围参数（`copyOfRange`、`substring`、`fill`、Stream 的 `limit`…）几乎都是"左闭右开"，记住这个约定能省下不少调试时间。

### 11.5.4 binarySearch() —— 二分查找（重要前提：数组必须先排序！）

二分查找用起来只有一行，但有一个绝对不能忘的前提：**数组必须已经排好序**。下面先排序再查找：

```java
int[] arr = {95, 87, 92, 78, 88};
Arrays.sort(arr); // 必须先排序！
// 排序后 arr = [78, 87, 88, 92, 95]
int index = Arrays.binarySearch(arr, 92);
System.out.println("92在数组中的索引：" + index); // 3
```
...
两个数组「看起来一样」，用 `==` 和 `equals()` 却都会得到 `false`，只有 `Arrays.equals()` 才会按内容比较：
...
| `copyOf()` | 复制并可改变长度 |
| `copyOfRange()` | 复制指定范围 |
| `asList()` | 把数组转成 `List`（定长视图，不能增删） |
| `compare()` | 按字典序比较两个数组（JDK 9+） |
| `mismatch()` | 找出两个数组第一个不同的位置（JDK 9+） |
| `setAll()` | 用一段逻辑统一生成每个元素（JDK 8+） |
| `stream()` | 把数组转成流，配合 Stream API 使用（JDK 8+） |
| `hashCode()` | 计算数组的哈希值 |
...
> 💡 **边界检查发生在运行时**：编译器不可能知道你算出来的索引是多少，所以越界不是编译错误。JVM 会在**每次访问数组元素时做一次边界检查**，一旦越界立刻抛出 `ArrayIndexOutOfBoundsException`。这确实是性能开销，但换来了安全——C 语言不做检查，越界会悄悄读写到相邻内存，轻则数据错乱，重则程序崩溃甚至被利用成安全漏洞。记住：**索引从 0 开始，最大是 `length - 1`**！每次写 `arr[i]` 前，先想想 `i` 的最大值是多少。

### 11.6.2 空指针异常（NullPointerException）

第二种高频错误是空指针：数组变量存在，但它指向的数组对象还不存在（或者已经被置为 `null`）：
...
初始化列表的语法限制值得再强调一次，下面是最容易写错的两种形式：
...
创建二维数组时，`new int[行][列]` 的顺序不能颠倒，也不能只写列数：
...
这是初学者最容易误判的一段代码，三种比较方式结果完全不同，务必记住：
...
基本类型的数组会「自动填 0」，包装类型的数组却是「自动填 null」，这个差异会直接导致空指针异常：
...
for-each 只能读、不能改。下面这段代码是新手最容易写出「看似改了其实没改」的典型：
