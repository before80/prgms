...
下面这些是写代码时一天要按上百次的键，值得花十分钟专门练一遍（macOS 上把 `Ctrl` 换成 `⌘`、`Alt` 换成 `⌥`）。
...
在几千个文件之间找代码，靠目录树翻是翻不完的——让光标替你跑腿。
...
IDEA 把大量样板代码做成了"实时模板"：敲几个字母再按 Tab，骨架就长出来了。
...
调试器的几个键位是肌肉记忆级别的，顺序记住「F8 走、F7 进、Shift+F8 出、F9 冲」就不容易乱。
...
重构键的共同点是：改一个名字，所有引用一起改，还顺手帮你更新注释和字符串。
...
记不住？IDEA 早就想到了。按 `Ctrl + Shift + A` 打开 action 搜索，输入 "keymap" 就能找到快捷键映射表。想看官方 PDF 速查卡，走菜单 `Help → Keymap Reference`（会打开 JetBrains 官方的快捷键 PDF）。在 `Settings → Keymap` 里还能给任意动作改键，或者切换成 Eclipse、VS Code 的键位方案。

> **小技巧**：切换快捷键方案后，本文的按键可能对不上——先确认右上角用的是 Windows/Linux 还是 macOS 方案。
...
**操作方法**：在断点上**右键**，弹出的浮层里直接就有 **Condition** 输入框，写入条件表达式即可，例如 `i == 999`。也可以在断点设置对话框（`Ctrl + Shift + F8`）里填。

> ⚠️ 别记混了：`Shift + F8` 是「步出当前方法」（Step Out），不是打开断点设置。
...
**操作方法**：直接在**方法声明那一行**的左侧装订区点击，确认后就是一个方法断点（图标是红色的方法形）。也可以在 `Ctrl + Shift + F8` 的 Breakpoints 对话框里按 `+` → **Java Method Breakpoints** 精确指定。

> **妙用**：当你怀疑某个接口被错误调用时，方法断点比在实现里乱打断点更精准。

> ⚠️ **性能提醒**：方法断点会让 JVM 以「解释模式」执行该方法（现代 JDK 上由调试器做字节码插桩），比普通行断点慢得多。用完记得关掉，别一直挂在代码里。
...
> **进阶技巧**：Watch 里可以输入复合表达式，比如做逻辑判断 `user != null && user.getAge() > 18`；要写多行可以在 Evaluate Expression 对话框里用 `Shift + Enter` 换行。

> ⚠️ **别把 Watch 当只读工具**：Watch 和 Evaluate Expression 执行的是**真实的代码**，不只是"看"。你完全可以写 `user.setName("x")`、`list.add(...)`、`counter++`，这些副作用会实打实地作用在被调试的进程上，而且调试结束后不会自动撤销。所以：**除非你就是想临时改状态来验证某条分支，否则只写没有副作用的查询表达式。**
...
每次重构，IDEA 都会记录操作历史。如果搞乱了，走菜单 `Refactor → Refactoring History`（重构历史）就能看到所有记录的步骤，逐步撤销或整体回滚。

> **黄金法则**：重构前记得先提交（commit）代码到 Git！IDEA 的重构历史和 Git 的本地历史（`Local History`）都只在 IDE 内部，换台机器就没了；只有 Git 的提交才是真正的"后悔药"。
...
> **提醒**：该插件近年更新频率明显下降，新版 IDEA 自带的 Darcula、Light、High Contrast 主题已经够用，也可以在 Marketplace 里挑一款还在活跃维护的主题插件。
...
#### 10. Spring Boot Assistant / Spring（Spring 生态辅助）

> **功能**：在 application.yml/properties 文件中提供属性自动补全，支持 Spring Boot 2.x/3.x 配置。
> **推荐理由**：Spring Boot 的配置项多如牛毛，有了它配置就像查字典一样简单。IDEA 旗舰版自带 Spring 支持；社区版可以装 Spring Boot Helper 这类社区插件。
...
#### 12. Redis Helper / Iedis（Redis 可视化）

> **功能**：在 IDEA 内直接查看和操作 Redis 数据。
> **推荐理由**：不用切换到 Redis CLI 或第三方客户端，一个 IDE 全搞定。IDEA 旗舰版的 Database 工具窗口原生支持 Redis（Ultimate 独有）；社区版则靠这类插件补齐。
...
1. **快捷键**：快捷键是 IDEA 高手与普通用户的分水岭。必须掌握的高频操作包括代码生成（`Alt + Insert`、`sout`/`psvm`）、代码导航（`Ctrl + B`、`Ctrl + Alt + B`）、重构（`Shift + F6`、`Ctrl + Alt + M`）以及调试（`F7`/`F8`/`F9`）的系列快捷键。记住，键盘永远比鼠标快。
