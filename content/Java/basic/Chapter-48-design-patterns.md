...
> 📌 **本章代码的组织方式**：设计模式天生涉及"多个类互相配合"，所以本章的示例常常分成好几个代码块来展示（一个类一块）。阅读时请把它们看作**同一个包里的多个 `.java` 文件**：`javac *.java` 一起编译，然后 `java` 运行带 `main` 的那个类即可。如果某个类在前面已经定义过，后面的代码块会直接使用它，不再重复定义。
