...
这一行声明了一个**类**（class）。类是什么？你可以把它想象成一个"容器"用来装代码，也可以说它是造对象的"蓝图"（blueprint）。Java 的代码必须写在类里，所以理解类是从第一行代码就开始的事。（严格来说 Java 并非"一切皆对象"——`int`、`double` 等基本类型就不是对象，这一点后面会细说。）
...
| `public` 类 | 任何地方都能访问（前提是能 import 到这个包） |
| 非 `public`（包级私有）类 | **只要在同一个包里**，其他文件也能访问；包外的代码访问不到 |

> ⚠️ **容易搞错的一点**：非 `public` 类**不是**"只有本文件能用"。它的可见范围是**整个包（package）**——同一个包里的其他 `.java` 文件照样可以用它，只是包外看不见。所以准确的说法是"包级私有"（package-private），而不是"文件私有"。
