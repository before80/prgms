...
下面这张图是 6 个月的**月度拆解**：前 3 个月打基础（Java 语法 → 核心类库），第 4 个月补数据库，第 5 个月补 Web，第 6 个月用 Spring Boot 做项目并开始投简历。每个方块里都是当月的主线任务，具体章节可以对照本教程前面的内容。

有一处需要提醒：图里的 **Servlet / JSP** 现在已经是"了解即可"的内容。它们是新框架底层的运行机制，但今天几乎没有新项目直接写 Servlet，你只需要知道"请求是怎么进到 Spring MVC 的"就够了，别在它们身上花太多时间。
...
从初级到中级，能力增长不再是"多学一个语法"，而是**在五个方向上同时加深**：编码能力、工具链、数据库、框架原理、并发与分布式。下面这张图列出的每一项，都值得你在 1~2 年里逐个啃透。
...
**什么是CAP定理？**
分布式系统不可能同时满足：一致性（Consistency）、可用性（Availability）、分区容错性（Partition tolerance）。

很多人把它背成"三选二"，这个说法并不准确：网络分区（P）不是你能主动"不要"的选项——只要机器之间靠网络通信，分区就一定会发生。严格的说法是：**平时系统可以同时满足 C 和 A；一旦发生分区，你只能保留其中一个**（CP 或 AP）。注册中心里 ZooKeeper 偏向 CP、Eureka 偏向 AP，就是这条权衡的具体体现。
...
中级到高级的跨越，主要发生在"技术深度、架构能力、工程能力、软技能、业务能力"这五个层面。下面的图可以当成一份自检清单：哪一栏你看得懂但讲不清，那就是下一步要补的地方。
