+++
title = "02-cargo build"
date = 2026-07-30T14:49:00+08:00
weight = 47
type = "docs"
description = "cargo-build(1) 编译当前包"
isCJKLanguage = true
draft = false
...
> 原文链接: [https://doc.rust-lang.org/cargo/commands/cargo-build.html](https://doc.rust-lang.org/cargo/commands/cargo-build.html)
...
<dd class="option-desc"><p>仅构建指定的包。SPEC 格式见 <a href="../manifest-commands/06-cargo-pkgid.md">cargo-pkgid(1)</a>。此标志可指定多次，并支持常见的 Unix glob 模式，如 <code>*</code>、<code>?</code> 和 <code>[]</code>。不过，为避免 shell 在 Cargo 处理之前意外展开 glob 模式，必须用单引号或双引号括住每个模式。</p>
</dd>
...
<dd class="option-desc"><p>构建工作空间中的所有成员。</p>
</dd>
...
若未给出目标选择选项，`cargo build` 将构建所选包的所有二进制与库目标。若二进制目标缺少其 `required-features`，则会被跳过。
...
<dd class="option-desc"><p>使用给定的配置文件构建。
...
<dd class="option-desc"><p>将最终产物复制到此目录。</p>
<p>此选项不稳定，仅在
<a href="https://doc.rust-lang.org/book/appendix-07-nightly-rust.html">nightly 通道</a>
可用，并需要 <code>-Z unstable-options</code> 标志才能启用。
...
其中之一构建失败，则 <code>cargo build -j1</code> 可能构建也可能不构建成功的那个
（取决于 Cargo 先运行哪一个），而 <code>cargo build -j1 --keep-going</code> 肯定会运行两者，
即便先运行的那个失败。</p>
</dd>
...
## 参见 {#see-also}
[cargo(1)](../../general-commands/01-cargo/)、[cargo-rustc(1)](../12-cargo-rustc/)
