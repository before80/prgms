+++
title = "12-cargo rustc"
date = 2026-07-30T14:49:00+08:00
weight = 57
type = "docs"
description = "cargo-rustc(1) 向 rustc 传额外参数"
isCJKLanguage = true
draft = false
...
> 原文链接: [https://doc.rust-lang.org/cargo/commands/cargo-rustc.html](https://doc.rust-lang.org/cargo/commands/cargo-rustc.html)


## 名称 {#name}
cargo-rustc --- 编译当前包，并向编译器传递额外选项

## 大纲 {#synopsis}
`cargo rustc` [_options_] [`--` _args_]

## 描述 {#description}
将编译当前包（或由 `-p` 指定的包）的指定目标及其所有依赖。指定的 _args_ 都会传给最终的编译器调用，而不会传给任何依赖。注意：编译器仍会无条件地收到诸如 `-L`、`--extern` 和 `--crate-type` 等参数，指定的 _args_ 只是追加到编译器调用中。

rustc 标志文档见 <https://doc.rust-lang.org/rustc/index.html>。

当提供额外参数时，此命令要求仅编译一个目标。若当前包有多个可用目标，必须使用 `--lib`、`--bin` 等过滤器选择要编译的目标。

若要将标志传给 Cargo 启动的所有编译器进程，请使用 `RUSTFLAGS`
[环境变量](../../../cargo-reference/07-environment-variables/) 或
`build.rustflags` [配置值](../../../cargo-reference/06-configuration/)。
...
<dd class="option-desc"><p>要构建的包。 SPEC 格式见 <a href="../manifest-commands/06-cargo-pkgid.md">cargo-pkgid(1)</a>。</p>
</dd>
...
若未给出目标选择选项，`cargo rustc` 将构建所选包的所有二进制与库目标。

若选择构建集成测试或基准测试，则会自动构建二进制目标。这样集成测试可以执行该二进制以演练并测试其行为。
...
<dd class="option-desc"><p>构建包的库。</p>
</dd>
...
<dd class="option-desc"><p>构建指定的示例。 此标志可指定多次，并支持常见的 Unix glob 模式。</p>
</dd>
...
<dd class="option-desc"><p>构建所有示例目标。</p>
</dd>
...
<dd class="option-desc"><p>构建指定的集成测试。 此标志可指定多次，并支持常见的 Unix glob 模式。</p>
</dd>
...
<dd class="option-desc"><p>构建所有在清单中设置了 <code>test = true</code> 标志的目标。默认包括作为单元测试构建的库与二进制目标，以及集成测试。请注意这也会构建任何所需依赖，因此 lib 目标可能会被构建两次（一次作为单元测试，一次作为二进制、集成测试等的依赖）。可通过在目标的清单设置中设置 <code>test</code> 标志来启用或禁用目标。</p>
</dd>
...
<dd class="option-desc"><p>构建指定的基准测试。 此标志可指定多次，并支持常见的 Unix glob 模式。</p>
</dd>
...
<dd class="option-desc"><p>构建所有在清单中设置了 <code>bench = true</code> 标志的目标。默认包括作为基准测试构建的库与二进制目标，以及 bench 目标。请注意这也会构建任何所需依赖，因此 lib 目标可能会被构建两次（一次作为基准测试，一次作为二进制、基准测试等的依赖）。可通过在目标的清单设置中设置 <code>bench</code> 标志来启用或禁用目标。</p>
</dd>
...
<dd class="option-desc"><p>构建所有目标。等价于指定 <code>--lib --bins --tests --benches --examples</code>。</p>
</dd>
...
<dd class="option-desc"><p>为指定的目标架构构建。此标志可指定多次。默认为主机架构。 triple 的一般格式为
...
<dd class="option-desc"><p>使用 <code>release</code> 配置文件构建优化后的产物。
...
<dd class="option-desc"><p>使用给定的配置文件构建。</p>
<p><code>rustc</code> 子命令会对以下命名配置文件采用特殊行为：</p>
<ul>
<li><code>check</code> — 与带 <code>dev</code> 配置文件的 <a href="03-cargo-check.md">cargo-check(1)</a> 命令相同的方式构建。</li>
<li><code>test</code> — 与 <a href="14-cargo-test.md">cargo-test(1)</a> 命令相同的方式构建，
启用测试模式（将启用测试并启用 <code>test</code> cfg 选项）。更多详情见
<a href="https://doc.rust-lang.org/rustc/tests/index.html">rustc 测试</a>。</li>
<li><code>bench</code> — 与 <a href="01-cargo-bench.md">cargo-bench(1)</a> 命令相同的方式构建，
类似于 <code>test</code> 配置文件。</li>
</ul>
...
<dd class="option-desc"><p>为给定的 crate 类型构建。此标志接受逗号分隔的 1 个或多个 crate 类型列表，允许的值与清单中配置 Cargo 目标的 <code>crate-type</code> 字段相同。可能的值见
<a href="../../cargo-reference/the-manifest-format/01-cargo-targets.md#the-crate-type-field"><code>crate-type</code> 字段</a>。</p>
<p>若清单包含列表，且提供了 <code>--crate-type</code>，
则命令行参数值将覆盖清单中的值。</p>
<p>此标志仅在构建 <code>lib</code> 或 <code>example</code> 库目标时有效。</p>
</dd>
...
其中之一构建失败，则 <code>cargo rustc -j1</code> 可能构建也可能不构建成功的那个
（取决于 Cargo 先运行哪一个），而 <code>cargo rustc -j1 --keep-going</code> 肯定会运行两者，
即便先运行的那个失败。</p>
</dd>
...
## 示例 {#examples}
1. 检查你的包（不包括依赖）是否使用了 unsafe 代码：

       cargo rustc --lib -- -D unsafe-code

2. 在 nightly 编译器上尝试实验性标志，例如打印每种类型的大小：
...
## 参见 {#see-also}
[cargo(1)](../../general-commands/01-cargo/)、[cargo-build(1)](../02-cargo-build/)、[rustc(1)](https://doc.rust-lang.org/rustc/index.html)
