+++
title = "02-cargo logout"
date = 2026-07-30T14:49:00+08:00
weight = 79
type = "docs"
description = "cargo-logout(1) 移除 API token"
isCJKLanguage = true
draft = false
...
> 原文链接: [https://doc.rust-lang.org/cargo/commands/cargo-logout.html](https://doc.rust-lang.org/cargo/commands/cargo-logout.html)


## 名称 {#name}
cargo-logout --- 在本地从注册表移除 API 令牌
...
对于默认的 `cargo:token` 凭证提供者，凭证存储在
`$CARGO_HOME/credentials.toml` 中，其中 `$CARGO_HOME` 默认为你主目录下的 `.cargo`。

若注册表指定了 credential-provider，则使用它。否则，
将从配置值 `registry.global-credential-providers` 中的提供者列表末尾开始尝试。

若未指定 `--registry`，则移除默认注册表的凭证
（由 [`registry.default`](../../../cargo-reference/06-configuration/#registrydefault) 配置，默认为
<https://crates.io/>）。

这不会在服务器上吊销令牌。若需要吊销令牌，
请访问注册表网站并按其说明操作（吊销 <https://crates.io/> 的令牌见
<https://crates.io/me>）。
...
## 参见 {#see-also}
[cargo(1)](../../general-commands/01-cargo/), [cargo-login(1)](../01-cargo-login/)
