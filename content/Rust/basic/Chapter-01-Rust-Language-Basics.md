...
Rust 通过**所有权系统**、**借用检查器**和**生命周期**等编译时机制，在 Safe Rust 中保证了：

- 内存安全：编译期杜绝空指针解引用、悬垂指针与数据竞争；
- 线程安全：编译期保证跨线程共享数据无数据竞争（Data-Race Free）；
- 同时，结合泛型单态化与 LLVM 优化，Rust 实现了零成本抽象，使高级抽象在释放模式下不引入额外运行时开销。
...
fn main() {
    ...
    // ✅ s1 仍然有效！
    println!("'{}' 的长度是 {}", s1, len); // 'hello' 的长度是 5
}
...
| 对比项   | C/C++                                               | Rust                                                      |
| -------- | --------------------------------------------------- | --------------------------------------------------------- |
| 内存安全 | ❌ 依赖开发者/工具链，无编译期 guarantee             | ✅ Safe Rust 编译期静态保证（无  Undefined Behavior 隐忧） |
| 抽象能力 | ✅ 极强（模板/RAII/STL），但易引发复杂性与运行时隐患 | ✅ 安全且零成本（编译器强制约束，抽象不付运行时代价）      |
| 编译时间 | ✅ 通常较快（C++ 重度模板/头文件项目除外）           | ⚠️ 较慢（语义检查+单态化+LLVM 优化，增量可缓解）           |
| 学习曲线 | ⚠️ 陡峭（特性多、历史包袱、隐式规则复杂）            | ⚠️ 更陡峭（所有权范式转变，但后期维护成本低）              |
...
// Go 的 channel 操作（send/recv/close）由 runtime 保证线程安全，
// 不会因并发操作 channel 本身而产生 data race。
// 但如果通过 channel 传递指针或共享可变状态，
// 仍需开发者保证数据访问的同步，否则仍可能出现 data race。
```
...
if __name__ == "__main__":
    result = sum_numbers(1_000_000_000)
    print(f"结果是: {result}")  # 如果 n 是 10 亿，那你就去喝杯咖啡吧 ☕
```
...
| 执行环境                  | 耗时（10 亿次求和） | 说明                     |
| ------------------------- | ------------------- | ------------------------ |
| 🖥️ 原生 Rust（编译成 exe） | ~0.3 秒             | CPU 直接跑，无中间层     |
| 🌐 WASM（Chrome 浏览器）   | ~0.4 秒             | 浏览器即时编译，轻微开销 |
| 🌐 JavaScript（for 循环）  | ~15 秒              | 解释执行 + 动态类型开销  |
...
- **gen 块（生成器）**：更强大的异步编程
- **unsafe extern blocks**：更安全的 **FFI（Foreign Function Interface）**，即**外部函数接口**
...
> “样板代码”（英文叫 **Boilerplate Code**）在编程界是个高频词，用大白话解释就是：
>
> > **跟你的核心业务逻辑毫无关系，但为了让程序能编译/运行，不得不反复复制粘贴的“行政手续代码”。**
>
> 就像你去银行办业务：你的核心需求是 `存钱`，但柜员让你先填 5 张表（姓名、身份证、手机号、风险提示、签名……）。这些表就是“样板代码”——**必不可少，但不产生实际价值，写多了还烦**。
>
>  ❌ 样板代码版（过去/现在常见写法）
>
> ```rust
...
> }
> 
> // 2. 为了能比较 user1 == user2，必须手动写 PartialEq
...
> }
> 
> // 3. 为了能 clone，必须手动写 Clone
> impl Clone for User { ... }
> ```
>
> 👆 上面这些代码**没写一行业务逻辑**，纯粹是为了满足 Rust 类型系统的“规矩”。如果项目有 50 个结构体，就要复制粘贴 150 遍。
>
>  ✅ 去掉样板代码后（现代 Rust 写法）
>
> ```rust
...
> `#![feature(gen_blocks)]` 这种语法**只能在 Rust Nightly（每夜构建版）编译器下运行**，在 Stable（稳定版）或 Beta 版中会直接报错。
>
> 🔍 为什么？一张表看懂 Rust 三个发布通道
>
> | 通道          | 更新频率              | 能否用 `#![feature(...)]` | 适用场景                         |
> | ------------- | --------------------- | ------------------------- | -------------------------------- |
> | **Stable** 🟢  | 每 6 周               | ❌ 绝对不能                | ✅ 生产环境、开源库、团队项目     |
> | **Beta** 🟡    | 每 6 周（预览下一版） | ❌ 不能                    | ✅ 提前测试兼容性                 |
> | **Nightly** 🔴 | 每天构建              | ✅ 可以                    | 🔬 实验新特性、语言贡献、个人探索 |
>
> 📌 **核心原则**：  
>
> > Stable 版承诺「**代码写了，5 年后还能编译通过**」；
