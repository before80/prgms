+++
title = "3.8 练习：Protobuf 解析"
date = 2026-08-11T11:30:00+08:00
weight = 158
type = "docs"
description = "练习：Protobuf 解析 — Comprehensive Rust"
isCJKLanguage = true
draft = false
rust_edition = "2024"
...
> 原文链接: [https://google.github.io/comprehensive-rust/lifetimes/exercise.html](https://google.github.io/comprehensive-rust/lifetimes/exercise.html)

# 3.8 练习：Protobuf 解析

本练习中，你将为
[protobuf 二进制编码](https://protobuf.dev/programming-guides/encoding/)
构建一个解析器。别担心，它比看起来简单！这展示了一种常见的解析模式：传递数据切片，底层数据本身从不被拷贝。

完整解析 protobuf 消息需要知道按字段编号索引的字段类型。这通常由 `proto` 文件提供。本练习中，我们把该信息编码进为每个字段调用的函数里的 `match` 语句。
...
proto 消息编码为一系列字段，一个接一个。每个字段实现为「标签」（tag）后跟值。标签包含字段编号（例如 `Person` 消息中 `id` 字段的 `2`）以及线类型（wire type），后者定义如何从字节流确定载荷。它们被组合成一个整数，如下面 `unpack_tag` 中的解码所示。

## Varint

包括标签在内的整数用一种称为 VARINT 的变长编码表示。幸好，下面已经为你定义了 `parse_varint`。

## 线类型

Proto 定义了若干线类型，本练习只用到其中两种。

`Varint` 线类型包含单个 varint，用于编码 `int32` 类型的 proto 值，例如 `Person.id`。

`Len` 线类型包含一个以 varint 表示的长度，后跟该数量的字节载荷。这用于编码 `string` 类型的 proto 值，例如 `Person.name`。也用于编码包含子消息的 proto 值，例如 `Person.phones`，其载荷包含子消息的编码。

## 练习

给定代码还定义了处理 `Person` 与 `PhoneNumber` 字段的回调，以及把消息解析为对这些回调的一系列调用。

你需要完成的是实现 `parse_field` 函数，以及为 `Person` 与 `PhoneNumber` 实现 `ProtoMessage` trait。
...
fn parse_field(data: &[u8]) -> (Field<'_>, &[u8]) {
    ...
    let (fieldvalue, remainder) = match wire_type {
        _ => todo!("根据线类型构建 Field，消费所需字节数。")
    };
    todo!("返回该字段以及任何未消费的字节。")
}
...
// TODO: 为 Person 与 PhoneNumber 实现 ProtoMessage。
...
> - 本练习中有多种 protobuf 解析可能失败的情况，例如数据缓冲区中剩余字节少于 4 时尝试解析 `i32`。在正常的 Rust 代码中我们会用 `Result` 枚举处理，但为简化，本练习在遇到任何错误时直接 panic。第 4 天我们会更详细地介绍 Rust 中的错误处理。
