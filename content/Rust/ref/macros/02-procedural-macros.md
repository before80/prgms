...
weight = 14
type = "docs"
description = "过程宏 — The Rust Reference"
isCJKLanguage = true
draft = false
+++
...
> 原文链接: [https://doc.rust-lang.org/reference/procedural-macros.html](https://doc.rust-lang.org/reference/procedural-macros.html)

r[macro.proc]
# 过程宏

r[macro.proc.intro]
*过程宏* 允许以执行函数的方式创建语法扩展。过程宏有三种风格：

* [函数式宏][Function-like macros] - `custom!(...)`
* [Derive 宏][Derive macros] - `#[derive(CustomDerive)]`
* [属性宏][Attribute macros] - `#[CustomAttribute]`

过程宏允许你在编译期运行对 Rust 语法进行操作的代码，既消费也产生 Rust 语法。你可以大致把过程宏想成从一份 AST 到另一份 AST 的函数。

r[macro.proc.def]
过程宏必须定义在 [crate 类型][crate type] 为 `proc-macro` 的 crate 的根中。这些宏不得从定义它们的 crate 中使用，只能在另一个 crate 中导入后使用。
...
r[macro.proc.result]
作为函数，它们必须要么返回语法，要么 panic，要么无限循环。返回的语法根据过程宏的种类替换或添加语法。panic 会被编译器捕获并变成编译错误。无限循环不会被编译器捕获，会使编译器挂起。

过程宏在编译期间运行，因此拥有与编译器相同的资源。例如，标准输入、错误和输出与编译器所能访问的相同。类似地，文件访问也相同。因此，过程宏具有与 [Cargo 的构建脚本][Cargo's build scripts] 相同的安全顾虑。
...
*`proc_macro` [属性][attributes]* 定义 [函数式][macro.invocation] 过程宏。
...
`proc_macro` 属性在 crate 根的 [宏命名空间][macro namespace] 中以与函数相同的名称公开定义该宏。
...
- 固有实现和 trait [实现][implementations]
...
将 *`proc_macro_derive` [属性][attribute]* 应用于函数会定义一个 *derive 宏*，可由 [`derive` 属性][`derive` attribute] 调用。这些宏被给予 [结构体][struct]、[枚举][enum] 或 [联合体][union] 定义的 token 流，并可在其后发出新的 [项][items]。它们也可以声明并使用 [derive 宏辅助属性][derive macro helper attributes]。
...
`proc_macro_derive` 属性在 crate 根的 [宏命名空间][macro namespace] 中公开定义该 derive 宏。

r[macro.proc.derive.output]
输入 [`TokenStream`] 是应用了 `derive` 属性的项的 token 流。输出 [`TokenStream`] 必须是（可能为空的）一组项。这些项被追加在输入项之后，位于同一 [模块][module] 或 [块][block] 内。

r[macro.proc.derive.attributes]
### Derive 宏辅助属性

r[macro.proc.derive.attributes.intro]
Derive 宏可以声明 *derive 宏辅助属性*，以在应用该 derive 宏的 [项][item] 的作用域内使用。这些 [属性][attributes] 是 [惰性的][inert]。虽然它们的目的是供声明它们的宏使用，但任何宏都可以看到它们。
...
当 derive 宏调用应用于一项时，该 derive 宏引入的辅助属性进入作用域：1) 用于应用于该项、且在词法上位于该 derive 宏调用之后的属性；以及 2) 用于应用于该项内部字段和变体的属性。

> **注意**
> rustc 目前允许在引入它们的宏之前使用 derive 辅助属性。这种乱序使用的 derive 辅助属性可能不会遮蔽其他属性宏。此行为已弃用，计划移除。
>
> <!-- ignore: requires external crates -->
> ```rust
> #[helper] // 已弃用，将来会变成硬错误。
...
> ```
>
> 更多细节见 [Rust issue #79202](https://github.com/rust-lang/rust/issues/79202)。
...
*`proc_macro_attribute` [属性][attributes]* 定义可用作 [外部属性][attributes] 的 *属性宏*。
...
> [!EXAMPLE]
> 这在编译器输出中显示属性宏所看到的字符串化 [`TokenStream`][`TokenStream`s]。
>
> <!-- ignore: test doesn't support proc-macro -->
> ```rust
> // my-macro/src/lib.rs
...
`proc_macro_attribute` 属性在 crate 根的 [宏命名空间][macro namespace] 中以与函数相同的名称定义该属性。
...
声明式 `macro_rules` 宏和过程宏对 token（或者说 [`TokenTree`][`TokenTree`s]）使用相似但不同的定义。
...
当 token 流传入和传出过程宏时，会考虑这两种定义之间的不匹配。注意下面的转换可能惰性发生，因此若 token 实际上未被检查，它们可能不会发生。
