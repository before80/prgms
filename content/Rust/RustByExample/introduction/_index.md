...
weight = 2
type = "docs"
description = "介绍 — Rust By Example"
isCJKLanguage = true
draft = false
+++
...
- [Hello World](../hello/) - 从经典的 “Hello World” 程序开始学习。

- [原生类型](../primitives/) - 学习有符号整型，无符号整型和其他原生类型。

- [自定义类型](../custom-types/) - 结构体 `struct` 和 枚举 `enum`。

- [变量绑定](../variable-bindings/) - 变量绑定，作用域，变量遮蔽。

- [类型系统](../types/) - 学习改变和定义类型。

- [类型转换](../conversion/) - 在不同类型之间进行转换，例如字符串、整数和浮点数。

- [表达式](../expression/) - 了解表达式以及如何使用。

- [流程控制](../flow-control/) - `if`/`else`，`for`，以及其他流程控制有关内容。

- [函数](../fn/) - 学习方法、闭包和高阶函数。

- [模块](../mod/) - 使用模块来组织代码。

- [Crate](../crates/) - crate 是 Rust 中的编译单元。学习创建一个库。

- [Cargo](../cargo/) - 学习官方的 Rust 包管理工具的一些基本功能。

- [属性](../attribute/) - 属性是应用于某些模块、crate 或项的元数据（metadata）。

- [泛型](../generics/) - 学习编写能够适用于多种类型参数的函数或数据类型。

- [作用域规则](../scope/) - 作用域在所有权（ownership）、借用（borrowing）和生命周期（lifetime）中起着重要作用。

- [特性 trait](../trait/) - trait 是对未知类型（`Self`）定义的方法集。

- [宏](../macros/) - 宏是一种编写代码来编写其他代码的方法，这称为元编程（metaprogramming）。

- [错误处理](../error/) - 学习 Rust 语言处理失败的方式。

- [标准库类型](../std/) - 学习 `std` 标准库提供的一些自定义类型。

- [标准库更多介绍](../std-misc/) - 更多关于文件处理、线程的自定义类型。

- [测试](../testing/) - Rust 语言的各种测试手段。

- [不安全操作](../unsafe/) - 了解如何进入不安全操作块。

- [兼容性](../compatibility/) - 处理 Rust 的演变和潜在的兼容性问题。
