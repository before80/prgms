...
weight = 99
type = "docs"
description = "我们已经在本书中使用过像 `println!` 这样的宏了，不过尚未深入探讨什么是宏以及它是如何工作的。宏（Macro）指的是 Rust 中一系列的功能：使用 `macro_rules!` 的声明宏，和三种过程宏。"
isCJKLanguage = true
draft = false
rust_version = "1.97.1"
...
> 原文链接: [https://doc.rust-lang.org/stable/book/ch20-05-macros.html](https://doc.rust-lang.org/stable/book/ch20-05-macros.html)
...
　　宏和函数的最后一个重要的区别是：在一个文件里调用宏 **之前** 必须定义它，或先将其导入，而函数则可以在任何地方定义和调用。

### 用 `macro_rules!` 编写用于通用元编程的声明宏 {#declarative-macros-with-macro_rules-for-general-metaprogramming}

　　Rust 最常用的宏形式是 **声明宏**（*declarative macros*）。它们有时也叫 “macros by example”、“`macro_rules!` 宏”，或干脆就叫 “macros”。其核心思路是：声明宏让我们写出类似 Rust `match` 表达式的代码。正如第 6 章所述，`match` 是一种控制结构：它接受一个表达式，把结果与各模式比对，再执行匹配分支中的代码。宏也会把“输入”与带代码的模式进行比较；这里的“输入”是传给宏的 Rust 源代码 token。模式与这些 token 比对，匹配成功后，对应代码会替换掉宏调用处的代码。这一切都发生在编译时。
...
　　`#[macro_export]` 注解表明只要导入了定义这个宏的 crate，该宏就应该是可用的。如果没有该注解，这个宏无法被导入使用。

　　接着使用 `macro_rules!` 和宏名称开始宏定义，且所定义的宏并 **不带** 感叹号。名字后跟大括号表示宏定义体，在该例中宏名称是 `vec` 。

　　`vec!` 宏的结构与 `match` 表达式类似。这里有一个分支模式 `( $( $x:expr ),* )`，后面是 `=>` 以及与该模式关联的代码块。若模式匹配，就会展开这段代码。由于该宏只有一个模式，也就只有一种有效匹配方式；其他任何模式分支都会导致错误。更复杂的宏可以有多个分支。
...
　　下面来看不同类型的过程宏。我们先从自定义派生宏入手，再说明其他形式之间的细微差别。
...
　　现在，我们已经引入了三个新的 crate：`proc_macro`、[`syn`][syn] 和 [`quote`][quote]。Rust 自带 `proc_macro` crate，因此无需将其加到 *Cargo.toml* 文件的依赖中。`proc_macro` crate 是编译器提供的 API，让我们能够在自己的代码中读取并操作 Rust 代码。
...
　　该函数首先将来自 `TokenStream` 的 `input` 转换为一个我们可以解释和操作的数据结构。这里正是 `syn` crate 大显身手之处。`syn` 中的 `parse` 函数获取一个 `TokenStream` 并返回一个表示解析出的 Rust 代码的 `DeriveInput` 结构体。示例 20-41 展示了从字符串 `struct Pancakes;` 中解析出来的 `DeriveInput` 结构体的相关部分：
...
　　此时，`cargo build` 应该都能成功编译 `hello_macro` 和 `hello_macro_derive` 。我们将这些 crate 连接到示例 20-37 的代码中来看看过程宏的行为！在 *projects* 目录下用 `cargo new pancakes` 命令新建一个二进制项目。需要将 `hello_macro` 和 `hello_macro_derive` 作为依赖加到 `pancakes` 包的 *Cargo.toml* 文件中去。如果你正将 `hello_macro` 和 `hello_macro_derive` 的版本发布到 [crates.io](https://crates.io/) 上，它们将是常规依赖；否则，则可以像下面这样将其指定为 `path` 依赖：
