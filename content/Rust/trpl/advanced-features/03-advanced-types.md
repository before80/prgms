...
weight = 97
type = "docs"
description = "Rust 的类型系统有一些我们曾经提到但尚未讨论过的特性。首先我们将从一般意义上讨论 newtype 并探讨它们作为类型为何有用。接着会转向类型别名，一个类似于 newtype 但有着稍微不同的语义的功能。我们还会讨论 `!` 类型和动态大小类型。"
isCJKLanguage = true
draft = false
rust_version = "1.97.1"
...
> 原文链接: [https://doc.rust-lang.org/stable/book/ch20-03-advanced-types.html](https://doc.rust-lang.org/stable/book/ch20-03-advanced-types.html)
...
　　本小节假设你已经阅读了之前的 [「使用 newtype 模式在外部类型上实现外部 trait」][newtype] 一节。
...
　　newtype 模式还可以隐藏内部实现。例如，可以提供一个封装了 `HashMap<i32, String>` 的 `People` 类型，用来储存人名以及相应的 ID。使用 `People` 的代码只需与我们提供的公有 API 交互即可，比如向 `People` 集合增加名字字符串的方法；这样这些代码就无需知道在内部我们将一个 `i32` ID 赋予了这个名字了。newtype 模式是一种实现第十八章 [「封装隐藏了实现细节」][encapsulation-that-hides-implementation-details] 一节中讨论的隐藏实现细节的轻量级封装方法。

### 类型同义词与类型别名 {#type-synonyms-and-type-aliases}
...
　　在函数签名和类型注解中到处书写这个冗长的类型既乏味又容易出错。想象一下有一个项目，到处都是像示例 20-25 那样的代码。
...
　　Rust 有一个叫做 `!` 的特殊类型。在类型理论术语中被称为 *empty type*（空类型），因为它没有值。我们更倾向于称之为 **Never 类型**（*never type*）。这个名字描述了它的作用：在函数从不返回的时候充当返回值。下面是一个示例：
...
　　当时我们略过了这段代码中的一些细节。在第六章 [「`match` 控制流结构」][the-match-control-flow-construct] 一节，我们讨论过：`match` 的各个分支必须返回相同的类型。所以下面的代码是行不通的：
...
　　Never 类型在 `panic!` 宏中也很有用。还记得 `Option<T>` 上的 `unwrap` 函数吗？它产生一个值或 panic。这里是它的定义：
...
### 动态大小类型和 `Sized` trait {#dynamically-sized-types-and-the-sized-trait}
...
　　那么该怎么办呢？在这种情况下，你已经知道答案：`s1` 和 `s2` 的类型是 `&str` 而不是 `str`。如果你回想第四章 [「字符串切片」][string-slices] 中提到，切片数据结构仅仅储存了开始位置和切片的长度。所以虽然 `&T` 是一个储存了 `T` 所在的内存位置的单个值，`&str` 则是**两个**值：`str` 的地址和其长度。这样，`&str` 就有了一个在编译时可以知道的大小：它是 `usize` 长度的两倍。也就是说，无论所引用的字符串多长，我们总是知道 `&str` 的大小。一般来说，这就是 Rust 使用动态大小类型的方式：它们有一些额外的元信息来储存动态信息的大小。这引出了动态大小类型的黄金法则：必须将动态大小类型的值置于某种指针之后。

　　可以将 `str` 与各种指针类型组合使用：例如 `Box<str>` 或 `Rc<str>`。事实上，你以前已经见过类似做法，只不过这里换成了另一种动态大小类型：trait。每个 trait 本身也是一种动态大小类型，我们可以通过 trait 的名字来引用它。在第十八章 [「使用 trait object 来抽象出共享行为」][using-trait-objects-to-abstract-over-shared-behavior] 一节中，我们提到过：为了把 trait 用作 trait 对象，必须把它放在某种指针之后，比如 `&dyn Trait` 或 `Box<dyn Trait>`（`Rc<dyn Trait>` 也可以）。
...
[encapsulation-that-hides-implementation-details]: ../../oop/01-what-is-oo/#encapsulation-that-hides-implementation-details
[string-slices]: ../../understanding-ownership/03-slices/#string-slices
[the-match-control-flow-construct]: ../../enums/02-match/#the-match-control-flow-construct
[using-trait-objects-to-abstract-over-shared-behavior]: ../../oop/02-trait-objects/#using-trait-objects-to-abstract-over-shared-behavior
[newtype]: ../02-advanced-traits/#implementing-external-traits-with-the-newtype-pattern
