...
> 原文链接: [https://doc.rust-lang.org/nomicon/vec/vec-final.html](https://doc.rust-lang.org/nomicon/vec/vec-final.html)
...
impl<T> RawVec<T> {
    ...
    fn grow(&mut self) {
        ...
        let (new_cap, new_layout) = if self.cap == 0 {
            ...
            // 这不会溢出，因为 self.cap <= isize::MAX。
            let new_cap = 2 * self.cap;
            (new_cap, Layout::array::<T>(new_cap))
        };

        // `Layout::array` 检查分配字节数在 1..=isize::MAX 内，
        // 否则会报错。由于上述条件，不可能分配 0 字节。
