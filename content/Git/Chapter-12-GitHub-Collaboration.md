...
```mermaid
graph TD
    A[1. Fork项目] --> B[2. git clone 你的Fork]
    B --> C[3. git remote add upstream]
    C --> D[4. git checkout -b feature-branch]
    D --> E[5. 修改代码并 git commit]
    E --> F[6. git push origin feature-branch]
    F --> G[7. 在GitHub创建PR]
    G --> H{需要修改?}
    H -->|是| I[本地修改并推送]
    I --> G
