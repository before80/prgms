...
date = 2026-04-03T15:31:07+08:00
weight = 1
type = "docs"
description = ""
isCJKLanguage = true
...
Every plugin needs a manifest, even with no config. See [Manifest](https://docs.openclaw.ai/plugins/manifest) for the full schema. The canonical ClawHub publish snippets live in `docs/snippets/plugin-publish/`.
...
OpenClaw also checks ClawHub before npm for bare package specs like `@myorg/openclaw-my-plugin`.**In-repo plugins:** place under the bundled plugin workspace tree — automatically discovered.
...
The `/approve` command handles both exec and plugin approvals with automatic fallback. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.See [SDK Overview hook decision semantics](https://docs.openclaw.ai/plugins/sdk-overview#hook-decision-semantics) for details.
