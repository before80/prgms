...
> 想象一下，你去图书馆看书，图书馆规定"只准看不准改"。const成员函数就是这样的规则——只让你读取数据，不让你搞破坏！
...
> 你知道C++里初始化和赋值是两回事吗？就像"从零开始创业"和"继承家业后改造"是完全不同的剧本！**成员初始化列表**就是让你选择用哪种方式迎接你的成员变量。
...
public:
    ...
    // this 关键字作为第一个显式参数，self 是参数名（在函数体内使用）
    // self 就相当于 *this，但以显式参数的形式出现，语法更透明
    // 调用方式不变：c.increment(); 仍然是 oop 调用，只是内部实现换了写法
    //
    // 对比一下两种写法的效果：
