...
int main() {
    ...
    // 注意：如果要读写二进制数据，可以加上 std::ios::binary
...
// 编译：g++ -std=c++17 31_06_path.cpp && ./a.out
// 注意：GCC 10+ 已内置 filesystem 支持，无需额外链接 -lstdc++fs
...
```cpp
// 文件系统操作函数
// 编译：g++ -std=c++17 31_06_filesystem.cpp && ./a.out
...
> 2. `std::print`默认向stdout输出，且在输出到终端时会自动刷新缓冲区
