...
> 简单记：取模结果的符号，跟被除数（左边那个）保持一致。被除数是正数，结果就是正数；被除数是负数，结果就是负数。
...
> **重要提醒**：对于基本类型，`x += y`和`x = x + y`的结果**完全相同**。但复合赋值**只计算一次**左侧表达式的值（这对`arr[index++]`这种有副作用的表达式很重要），而`x = x + y`会计算两次！此外，对于重载了运算符的用户自定义类型，行为可能不同。
...
bool empty = nullptr;  // nullptr -> bool false
bool nonEmpty = "";  // 非空字符串指针 -> bool true
```
