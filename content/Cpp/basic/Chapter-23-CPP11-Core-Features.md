...
int main() {
    ...
    // 如果编译器不支持C++14，可以手动实现，或直接用 std::unique_ptr<T>(new T(args...))
    // 注意：本章代码若使用make_unique，需确保编译器支持C++14
    auto up1 = std::make_unique<int>(42);  // 在堆上创建一个int，值为42（C++14）

    // 方法2：直接用构造函数（不推荐，容易写错）
    std::unique_ptr<int> up2(new int(42));  // C++11方式
...
int main() {
    ...
    // ========== 函数返回值类型推导（C++14）==========
    // 注意：以下代码需要C++14标准，C++11不支持函数返回值的auto推导
    // auto add(int a, int b) {
    //     return a + b;
    // }
...
int main() {
    ...
    std::vector<int> v3(100);  // 错！这是100个默认值0的vector
    ...
    std::vector<int> v5();   // 编译错误！这是函数声明
    std::vector<int> v6{};   // 默认构造：空vector
    std::vector<int> v7;     // 默认构造：空vector
...
int main() {
    ...
    // 问题2：不同枚举的值可以比较
    if (Red == High) {  // 编译错误？还是能通过？
        // 实际上可以编译！因为它们都被转成int比较
    }
