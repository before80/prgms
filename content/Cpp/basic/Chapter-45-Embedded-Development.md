...
> 📦 **运行说明**：这是面向 **STM32 单片机 + HAL 库**的工程示例，只能在对应的交叉编译工具链和开发板上编译运行。`SystemClock_Config()`、`GPIO_Config()`、`delay_ms()`、`getTick()` 等都不是标准库函数，而是 HAL 库/板级支持包提供的接口，所以在 PC 上直接编译会提示"未声明的标识符"。请把它当作结构参考来读。
