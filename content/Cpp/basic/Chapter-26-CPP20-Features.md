...
// 注意：std::floating_point 是 C++23 概念，C++20 中可用 std::is_floating_point_v
// 这里用 std::floating_point 仅为演示，实际项目请注意编译器标准版本
template<typename T>
...
[[likely]] int processPositive(int x) {
...
[[unlikely]] void slowOperation() {
    // [[unlikely]] 告诉编译器："这个分支很少执行，可以不那么优化"
    // 用于错误处理、边界情况等
    std::cout << "这是一个很少执行的分支（慢路径）" << std::endl;
}
