...
| VS2022 17.6+ | MSVC 14.36 | C++20概念、协程等 |
...
int main() {
    // std::cout 的 std 是 "standard" 的缩写，代表"标准"命名空间
    // :: 是作用域运算符，相当于在说"我要用 std 命名空间里的 cout"
...
```
其中`cl.exe`是MSVC的编译器驱动程序（compiler driver），负责调用编译器、链接器等整套工具链，`/EHsc`是异常处理选项，`/std:c++20`指定C++标准。
...
**方式一：LLVM-MinGW（推荐，最省心）**

来自LLVM项目的Windows版MinGW-w64，预编译好了，不用折腾。地址：https://github.com/llvm/llvm-mingw

**方式二：MSYS2（推荐给喜欢包管理器的同学）**

MSYS2自带pacman包管理器，安装GCC等工具非常方便。具体步骤见下一节"MSYS2环境配置"。

**方式三：独立安装器（mingw-get，不推荐）**

如果你非要单独安装MinGW-w64，可以去 SourceForge 下载 `mingw-get-setup.exe`：
1. 下载并运行安装器
2. 选择安装路径（建议 `C:\mingw64`，路径别有中文和空格）
3. 在包管理器里勾选 `mingw32-gcc-bin`、`mingw32-gcc-g++-bin` 等
4. 点击"应用更改"→"安装"

> ⚠️ 注意：mingw-get方式安装的版本通常比较老，且后续更新麻烦。**强烈建议使用LLVM-MinGW或MSYS2方式**。
...
| C++标准支持 | 跟进快 | 也很快 |
| 平台支持 | 全平台 | 全平台 |
| 许可证 | Apache 2.0 / LLVM (更宽松) | GPL 3 (Copyleft) |
...
int main() {
    ...
    // std::endl 是 "end line" 的缩写，表示"结束当前行并换行"
...
| `std::endl` | 换行符（并刷新输出缓冲区） | endl = end line |
...
**Visual Studio** 是微软的旗舰IDE，功能极其全面。在Windows平台，它是C++开发的主流选择之一。
...
# 或者用MSYS2（推荐，安装clang-tools-extra包含clang-format和clang-tidy）
