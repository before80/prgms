...
│flex-end:                 [项目][项目][项目]│
│center:             [项目][项目][项目]      │
│space-between:[项目]      [项目]      [项目]│
│space-around:   [项目]    [项目]    [项目]  │
│space-evenly:    [项目]   [项目]   [项目]   │
└────────────────────────────────────────────┘

提示：space-between 首尾贴边、中间均分；space-around 每个项目左右各有一份间距
（所以项目之间是边缘间距的两倍）；space-evenly 则是所有间距完全相等。
```
...
> ⚠️ `order` 只改变**视觉顺序**，不改变 DOM 顺序。用 Tab 键导航时，焦点仍然按 HTML 里的顺序走；屏幕阅读器也按 DOM 顺序朗读。所以：**纯粹为了视觉效果调整顺序可以，但不要让"视觉顺序"和"阅读顺序"严重脱节**——那种情况下应该直接改 HTML 结构。
...
> 📱 **移动端注意**：`height: 100vh` / `min-height: 100vh` 在手机浏览器里会把地址栏占掉的高度也算进去（称为 `100vh` 问题），结果是页面底部被顶出屏幕、刚好差一条地址栏的高度。解决办法是改用**动态视口单位**：
>
> ```css
> .page-layout {
>   min-height: 100dvh;   /* dvh = 动态视口高度，会随地址栏收放实时变化 */
>   /* 也可以写 min-height: 100svh; 取"最小的"视口高度，最保守 */
> }
> ```
>
> 配合 `min-height: 100vh;` 写两行做降级兼容效果更好（不支持 `dvh` 的老浏览器会用 `vh` 那一行）。
...
### 23.5.5 最坑的一个：flex 项目的 `min-width` 默认是 `auto`

这是 Flexbox 里"最著名的一个坑"。flex 项目在主轴方向上的 `min-width`（column 方向则是 `min-height`）**默认值是 `auto` 而不是 `0`**，意思是"我不愿意被压到比内容还窄"。后果是：`flex: 1` 的项目依然可能撑破容器、省略号（`text-overflow: ellipsis`）死活不出现、图片溢出不收缩……

```css
/* 症状：这个卡片明明 flex: 1，却把容器撑破了 */
.card {
  flex: 1;
  /* 内容里有一段很长的没有空格的文字或一张大图 */
}
...
/* 如果主轴是垂直方向（flex-direction: column），改的是 min-height */
.column-item {
  flex: 1;
  min-height: 0;
}
...
> 记住这条经验：**只要你在 flex 项目里想用 `overflow: hidden` + `text-overflow: ellipsis` 做文字截断，就必须同时给它加上 `min-width: 0`。** 单纯写 `overflow: hidden` 是不够的——这是无数人排查半天才找到答案的经典问题。

### 23.5.6 `flex: 1` 里的 `0%` 不能省成 `0`

```css
.a { flex: 1; }        /* ✅ 浏览器展开为 flex: 1 1 0% */
.b { flex: 1 1 0; }    /* ⚠️ 在老版本 Safari 上会被当成 flex-basis: auto，表现和 flex: 1 不一致 */
```

规范里 `flex-basis: 0` 和 `0%` 是等价的，但早期 Safari / IE 对无单位的 `0` 有实现差异。**要写就写 `flex: 1`，或者老老实实写 `flex: 1 1 0%`**，不要写中间那个 `flex: 1 1 0`。

另一个容易忽略的细节：`flex: 1 1` 这种省略 `flex-basis` 的写法，等于 `flex-basis: 0%`（不是 `auto`）；而只写 `flex: 1` 又会被展开成 `flex: 1 1 0%`。所以"省略"和"默认"是两回事，别靠记忆，直接写全。
...
> 再强调三件事：**flex 项目的 `min-width` 默认是 `auto`**（要截断文字就得加 `min-width: 0`）；**`gap` 只在项目之间产生间距，不会在容器边缘留白**；**`order` 只改视觉顺序，不改 Tab 键和读屏顺序**。
