...
> ⚠️ **移动端 100vh 的坑**：在手机上，`100vh` 是"地址栏收起时"的视口高度，所以标题栏还在的时候，`height: 100vh` 的元素会被顶出去一截，出现讨厌的滚动条。现代做法是改用**动态视口单位**：
>
> ```css
> .hero-section {
>   min-height: 100dvh;  /* 随地址栏收放动态变化，才是真正的"一屏高" */
> }
> ```
>
> 相关的还有 `svh`（地址栏展开时的小视口高度）和 `lvh`（地址栏收起时的大视口高度）。兼容性兜底可以写两行：先 `min-height: 100vh;`，再用 `min-height: 100dvh;` 覆盖（老浏览器会忽略它不认识的那行）。
