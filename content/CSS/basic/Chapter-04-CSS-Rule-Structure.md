...
/* 为什么 px 是最常用的？ */
/* CSS 里的 px 叫"CSS 像素"，按定义 1px = 1/96 英寸，与设备无关 */
/* 它不是屏幕上的物理像素点！在 Retina/高清屏上，1 个 CSS 像素 */
/* 会由 2 个或 3 个物理像素共同绘制（devicePixelRatio）*/
/* 所以同样写 16px，在高清屏上看起来更"细腻"，尺寸却一致 */
```
...
| 单位 | 参考对象 | 说明 |
|------|----------|------|
| em | 见下方说明 | `font-size` 用 em 时相对父元素，其他属性用 em 时相对**自己**的 font-size |
| rem | 根元素 font-size | 相对于根元素（html）字体大小 |
| vw | 视口宽度 | 1vw = 视口宽度的 1%（含滚动条占位） |
| vh | 视口高度 | 1vh = 视口高度的 1%（移动端受地址栏影响，见下） |
| vmin | vw 和 vh 中较小者 | 用于适配不同屏幕比例 |
| vmax | vw 和 vh 中较大者 | 用于适配不同屏幕比例 |
| ch | 数字"0"的宽度 | 常用于输入框最小宽度；非等宽字体下只能粗略估计 |
| ex | 小写字母 x 的高度 | 很少使用 |
| dvh / svh / lvh | 动态/最小/最大视口高度 | 移动端地址栏收起-展开时的三种高度，见下 |

> **em 的参考对象**：这是最容易记错的一条。`em` 的含义是"当前元素的 font-size"，而 `font-size` 在计算时又没法参考自己，于是退一步参考父元素的 font-size。所以：
>
> - 写 `font-size: 2em` → 相对**父元素**的字号；
> - 写 `padding: 1.5em`、`margin: 1em`、`width: 10em` → 相对**当前元素自己**的字号（不是父元素！）。
...
.child {
  ...
  padding: 1.5em;            /* 注意！padding 的 em 相对自己：32px * 1.5 = 48px */
}
...
/* 视口单位，常用于全屏布局 */
.full-screen {
  height: 100vh;            /* 100vh = 整个视口的高度，铺满一屏 */
  width: 100vw;             /* 100vw = 视口宽度 */
}

/* 两个非常经典的坑： */
/* 1. 页面有纵向滚动条时，100vw 会把滚动条那部分也算进去， */
/*    于是宽度比可见区域宽，导致多出一条横向滚动条。 */
/*    解决：需要满宽时用 width: 100%（相对父级）或 100dvw。 */
/* 2. 移动端浏览器地址栏会收起/展开，100vh 是"地址栏展开时"的高度， */
/*    第一个 100vh 元素可能被地址栏遮住一部分。 */
/*    解决：改用 dvh（动态，随地址栏变化）、svh（最小）、lvh（最大）。 */
.hero-modern {
  min-height: 100dvh;       /* 现代写法：始终等于当前可见视口高度 */
}
...
/* ========== ch ========== */
/* 相对于数字"0"的宽度（在等宽字体里等于任意一个字符的宽度） */
.username-input {
  min-width: 20ch;          /* 大约能显示 20 个数字/字母 */
  padding: 0.5rem 1rem;
...
/* 问题：颜色名不精确 */
/* 更准确地说：颜色名本身是精确的（规范写死了 red 就是 #ff0000）， */
/* 真正的缺点是数量有限、命名和实际用途容易脱节。 */
/* 解决方案：需要表达品牌色、深浅变化时使用十六进制、rgb() 或 hsl() */
...
/* 8 位十六进制（带透明度，CSS Color Level 4）：#RRGGBBAA */
/* 最后两位是 alpha：00 = 全透明，ff = 全不透明 */
.color11 { color: #ff0000ff; }    /* 红，alpha = 255/255 = 1（完全不透明） */
.color12 { color: #ff000080; }    /* 红，alpha = 128/255 ≈ 0.5（半透明） */
.color13 { color: #ff000033; }    /* 红，alpha = 51/255 = 0.2（即 80% 透明） */

/* 4 位简写（带透明度）：#RGBA，每一位都要重复一次 */
.color14 { color: #f00f; }        /* 展开 = #ff0000ff，不透明的红 */
.color15 { color: #f008; }        /* 展开 = #ff000088，alpha ≈ 0.53 */
```
...
```css
/* 1. rgba() 和 hsla() 现在只是 rgb() / hsl() 的别名，行为完全一样。
      新代码统一写 rgb() / hsl() 即可，alpha 作为第 4 个参数。 */
.a { color: rgb(255 0 0 / 0.5); }   /* 空格分隔 + 斜杠：现代语法 */
.b { color: rgb(255, 0, 0); }       /* 逗号分隔：传统语法，仍然有效 */

/* 2. 现在的 rgb() 还支持 0-100% 的百分比通道，不必再记住 0-255 */
.c { color: rgb(100% 0% 0% / 50%); }  /* 同样是半透明红 */
```
...
/* H 色相：0-360 度，代表颜色在色轮上的位置；色轮角度对应如下 */
...
/* 前提：必须先在某个祖先元素上声明 color-scheme，
   否则 light-dark() 永远只走"亮色"分支 */
:root {
  color-scheme: light dark;   /* 允许这套配色在亮色/暗色下切换 */
}
...
> 关键前提：`light-dark()` 只有在 `color-scheme` 允许暗色时才会返回暗色值。如果页面没有设置 `color-scheme: light dark`（默认是 `normal`，等同只有亮色），那么无论系统是不是暗色模式，`light-dark()` 都只会返回第一个参数。所以正确的姿势是先把 `:root { color-scheme: light dark; }` 写上，再使用 `light-dark()`。
...
/* calc() 里最著名的坑：+ 和 - 两边必须有空格 */
.good { width: calc(100% - 40px); }   /* 有效：减号两侧有空格 */
.bad  { width: calc(100%-40px); }     /* 无效：被当成一个标识符，整条声明被丢弃 */
/* 乘除号 * / 不需要空格：calc(50% / 2)、calc(100px*2) 都合法 */
...
/* ========== rem()（注意：是"取余函数"，和 rem 单位不是一回事） ========== */
.rem-fn-demo {
  /* mod() 与 rem() 的区别只在负数上： */
  width: mod(-10px, 3px);   /* 2px：结果符号跟随除数 */
  width: rem(-10px, 3px);   /* -1px：结果符号跟随被除数 */
}
...
/* 历史限制：attr() 过去只能在 content 里使用。
   现在 CSS Values 5 已经允许在其他属性里使用，并可用 type() 指定类型，
   但浏览器支持还在推进中，旧浏览器仍需在 content 里用：
   width: attr(data-width px);   <-- 较新浏览器才支持 */
...
> ⚠️ **注释不能嵌套**，这是"用注释临时禁用一整块代码"时最容易翻车的地方：
>
> ```css
> /*
> .card {
>   color: red;      /* 这一行里已经有一对注释符号了 */
>   padding: 20px;
> }
> */
> ```
>
> CSS 的注释在遇到**第一个** `*/` 时就结束。上面这段里，"这一行里已经有一对注释符号了"那行的 `*/` 就成了整块注释的终点，之后的 `padding: 20px;` 和结尾的 `}` 都会变成"真实代码"，而最后多出来的 `*/` 又成了语法错误——于是本来只想临时禁用，结果页面样式被改坏。
>
> 需要整块禁用时有两个稳妥办法：
>
> - 把这一块移到文件末尾或单独的 `.bak` 文件里；
> - 用打包/编辑器插件（如 VS Code 的 `Ctrl + /`）逐行加 `/* */`，或者先删掉内部注释再整体注释。
...
graph TD
    ...
    C --> C1["修改代码时同步更新注释"]
    D --> D1["不要写明显的废话"]
    D1 --> D2["❌ 写一句'这是红色'来解释 color: red"]
    D1 --> D3["✓ 说明为什么用红色，例如它是品牌强调色"]
...
| 元素/伪元素 | 1 | `div`, `::before` |
| 组合器（` `、`>`、`+`、`~`） | 本身不计分 | `div > p` 的分值来自 `div` 和 `p`，空格和 `>` 不加分 |

> 📌 关于"1000 / 100 / 10 / 1"这套分值：它是社区里流传最广的**便于记忆的简化模型**，但严格来说并不准确。规范用的是三个计数器：
>
> - **A**：ID 选择器的个数；
> - **B**：类、属性、伪类的个数；
> - **C**：元素、伪元素的个数。
>
> 比较时把三者写成 `(A, B, C)`，**从左到右逐位比大小**，而不是把 10 个类"进位"成一个 ID。所以每个类选择器都带 10 分、10 个类等于 1 个 ID 的说法只是"看起来像"，用分值模型记住大概顺序没问题，真正的规则请以三计数器为准。第 33 章会完整展开这套规则。
...
下一章我们将学习 CSS 属性缩写与嵌套，包括 margin/padding 缩写、font 缩写、flex 缩写、CSS 嵌套语法等。这些技巧能让你的 CSS 代码更加简洁优雅。后面还有选择器、盒模型、布局、动画等大量内容在等着你，路还长着呢！
