...
int main() {
    ...
    printf("x %% y = %d\n", x % y);  // 输出: x % y = 1（注意 format 里的 %% 只打印一个 %）
    // 注意：printf 的格式串里要输出字面的 %，需要写 %%
...
>
> 因为 `%` 在 printf 里是"特殊身份"——用来标记格式说明符（如 `%d`、`%f`）。你想输出字面的 `%`，就得"转义"一下，写成 `%%`。这就像想说"老板姓李"，你得写"李老板"以免混淆一样。
...
int main() {
    ...
    // 不同编译器的"史前遗迹"
    // 某些老的 C89 编译器（向负无穷取整）会输出：-7 / 2 = -4
    // GCC/Clang（C99+，向零截断）输出：-7 / 2 = -3

    return 0;
}
```

> **C89 vs C99 的恩怨情仇**：C89 把负数除法/取余的结果交给各编译器自己定（implementation-defined）。于是有的编译器"向负无穷取整"（-7/2 = -4），有的"向零截断"（-7/2 = -3），同一份代码换个编译器结果就变了。到了 C99，标准委员会一拍桌子："都给我向零截断！"这才平息了江湖纷争。
...
int main() {
    ...
    // 右移：低位丢弃；常见的编译器对负数会"高位补符号位"（算术右移）
...
int main(void) {
    ...
    // 前置 ++i：先自增，再把新值作为表达式的值
    int pre = ++i;   // i 变成 6，pre 拿到 6
    printf("++i 的值 = %d, i = %d\n", pre, i);   // 输出: ++i 的值 = 6, i = 6

    // 后置 i++：先把旧值作为表达式的值，再自增
    int post = i++;  // post 拿到 6，然后 i 变成 7
    printf("i++ 的值 = %d, i = %d\n", post, i);  // 输出: i++ 的值 = 6, i = 7
...
> **⚠️ 常见坑：** 千万不要把自增/自减和同一个变量的读取塞进同一个表达式的多个参数里，比如
> `printf("%d %d\n", i++, i)` 或 `x = i++ + i++;`。C 标准规定："同一个变量在相邻两个序列点之间既被修改又被读取"属于**未定义行为**（Undefined Behavior），不同编译器、不同优化级别可能给出完全不同的结果（甚至同一份代码改个编译选项结果就变了）。上面故意拆成两条语句，就是为了避免这个坑——详见 5.13 节。
...
int main(void) {
    ...
    int pre = --i;   // i 变成 4，pre 拿到 4
    printf("--i 的值 = %d, i = %d\n", pre, i);  // 输出: --i 的值 = 4, i = 4

    int post = i--;  // post 拿到 4，然后 i 变成 3
    printf("i-- 的值 = %d, i = %d\n", post, i); // 输出: i-- 的值 = 4, i = 3
...
int main() {
    ...
    // const 修饰后变成"只读左值"：可以读取，不能赋值
    const int c = 10;
    // c = 20;  // 错误！不能给 const 变量赋值
    printf("c = %d\n", c);  // 正确：c 可以读取

    // 数组名在大多数场合会"退化"成指向首元素的指针，因此不能被赋值；
    // 而 arr[i] 才是货真价实的可修改左值，可以取地址、可以赋值。
    int arr[3] = {1, 2, 3};
    arr[0] = 100;  // 正确：arr[0] 是左值
    // arr = NULL;  // 错误！数组名不是可修改左值
    printf("arr[0] = %d\n", arr[0]);
...
> 📝 **术语小注：** "序列点"（sequence point）是 C89/C99 时代的说法。从 **C11 起**，标准改用了更精确的"**先序关系**"（sequenced before / unsequenced）来描述求值顺序——但底层思想没变：**同一个变量在"既没被排序、又被读写两次"时就是未定义行为**。为方便理解，本章沿用"序列点"这个更直观的旧说法。
