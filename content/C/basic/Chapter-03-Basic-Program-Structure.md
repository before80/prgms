...
`void` 在函数参数列表里表示"这个函数不接受任何参数"。

那么写成 `int main()`（空括号）行不行呢？**这取决于你用的 C 标准**，这里有一条容易踩的历史分界线：

| 标准 | `int main()` 的含义 |
|------|---------------------|
| C89 ~ C17 | "参数**未知**"（没有原型，编译器不检查实参） |
| **C23 及以后** | 等同于 `int main(void)`，即"**不接受任何参数**" |

> C23 采纳了提案 N2841《No Function Declarators without Prototypes》，规定**空参数列表就等同于只有 `void`**。也就是说，那个"空括号表示参数未知"的老规则在 C23 里已经被删掉了。
>
> 这件事现在很要紧：**GCC 15 开始默认就是 C23**。也就是说，同一段 `int main()` 在 GCC 14 和 GCC 15 下的语义并不一样。

不管在哪个标准下，**显式写 `void` 都是最好的习惯**——它把"零参数"这个意图明明白白写出来，不依赖标准版本的差异，也不会让读代码的人去猜。
...
> 📌 这 8 个阶段是 C 标准第 5.1.1.2 节明确规定的，而且标准特别强调：**这只是概念上的划分**，实际编译器完全可能把若干阶段合并在一起做（比如 GCC 的 `-E` 就把 1~4 阶段一口气做完）。

下面是每个阶段**准确**的职责（很多教程在这里讲错，请认准标准原文）：

| 阶段 | 做什么 |
|:---:|--------|
| 1 | 把物理源文件的字符映射到**源字符集**，并统一换行符 |
| 2 | 删除每一处"反斜杠 + 换行"，把物理行拼成**逻辑行** |
| 3 | 把源文件切成**预处理记号**（preprocessing token）和空白/注释；**每个注释替换成一个空格** |
| 4 | 执行预处理指令：宏展开、`#include`、条件编译……结束后所有预处理指令被删除 |
| 5 | 把字符常量和字符串字面量里的字符与转义序列，转成**执行字符集**中的对应字符 |
| 6 | 把**相邻的字符串字面量**拼接成一个 |
| 7 | 把预处理记号转成**记号**，做语法/语义分析，翻译成翻译单元 |
| 8 | 解析所有外部引用，链接库，生成可执行程序映像 |

### 3.5.1 字符映射与换行符标准化

**阶段1**：把源文件里的所有字符映射到**源字符集**（source character set），并且把不同平台的行尾标记统一成换行符。

**阶段2**：删除每一处**反斜杠 + 换行符**。这就是所谓的"续行"——`\` 把两行粘成一行，物理行由此变成逻辑行。

**阶段3**：把源文件切成**预处理记号**（preprocessing token）和空白序列。这个阶段有个很实用的细节：**每个注释会被替换成一个空格**，而不是"直接删掉"。

```c
int/**/x = 1;   // 注释换成空格 → int x = 1;   合法
int a = 1/**/2; // 注释换成空格 → int a = 1 2;  语法错误（不是 12！）
```

> 💡 这就解释了一个常见疑问：为什么 `a/**/b` 不会变成 `ab`？因为 `/**/` 在阶段 3 被换成了一个**空格**，所以它是 `a` 和 `b` 两个记号。想拼接记号得用 `##` 运算符，那是阶段 4 的宏展开干的事。
...
#### 三字符组（Trigraph）：已经进入历史的"化石"

三字符组是 **C89/C90** 引入的稀奇古怪的东西，目的是照顾那些字符集里没有某些符号的设备。简单说，就是用三个 ASCII 字符的组合来代表一个"不好直接输入"的字符。
...
> 😂 举个例子：`??=` 会被替换成 `#`，所以 `??=include <stdio.h>` 会被当成 `#include <stdio.h>`。听起来很荒谬？确实！
>
> 它的完整生命周期是这样的：
>
> | 标准 | 三字符组的地位 |
> |------|---------------|
> | C89/C90 | 正式引入，在**阶段 1** 完成替换 |
> | C99 | 标记为**过时（obsolescent）** |
> | C17 | 依然存在（GCC 需要额外加 `-trigraphs` 才启用） |
> | **C23** | **彻底删除**（提案 N2940《Remove Trigraphs??!》） |
>
> ⚠️ 这里要纠正一个流传很广的错误说法：**"C23 仍然识别三字符组，只是会给出诊断"是错的**。C23 的变更清单里明确列着"移除三字符组"，标准正文里有关三字符组的内容已被完全删掉——在 C23 下 `??=` 就是三个普通的问号和等号，不再有任何特殊含义。
...
**阶段5 ~ 阶段7**：这三个阶段合起来就是我们平时说的"**编译**"（compile）。标准对它们的定义是：

- **阶段5**：把字符常量、字符串字面量里的字符和转义序列，转成**执行字符集**（execution character set）中对应的字符。比如 `'\n'` 在这时候才真正对应到目标平台上"换行"那个字节值。
- **阶段6**：把**相邻的字符串字面量**拼接成一个。这就是为什么 `"abc" "def"` 等价于 `"abcdef"`。
- **阶段7**：把预处理记号转成**记号**（token），做完整的语法与语义分析，翻译成**翻译单元**（translation unit）。真正的"翻译"发生在这里——编译器在此生成汇编代码，并在此阶段报出绝大多数语法错误。

> ⚠️ **常见误解**：阶段 5 常被说成"编译成汇编"、阶段 6 被说成"汇编成目标文件"。这其实把**翻译阶段的编号**和**工具链的工序**混为一谈了。标准里的阶段 5/6 讲的都是"字符编码转换"和"字面量拼接"；"生成汇编"和"生成目标文件"在标准里是**阶段 7 之后由具体工具实现**的事情。

到了工具链层面，工序通常是这样的：

```mermaid
flowchart LR
    A[".c 源文件"] -->|"gcc -E（阶段1~4）"| B[".i 文件"]
    B -->|"gcc -S（阶段5~7，编译）"| C[".s 汇编文件"]
    C -->|"gcc -c（as 汇编器）"| D[".o 目标文件"]
```

### 3.5.5 链接 → 可执行文件

**阶段8**：**链接器**（linker）出场了。它把一个或多个目标文件（以及你用到的库文件）"拼装"在一起，解析所有**外部引用**（external reference）。

比如你调用 `printf`，链接器会去标准 C 库（libc）里找到 `printf` 的实现，把它的机器码合并进来。

最终生成**可执行文件**（executable）。在 Linux/macOS 上通常没有扩展名（默认叫 `a.out`），在 Windows 上是 `.exe`。
...
> 💡 C 语言里 PascalCase 通常用于**类型名**（typedef、struct tag）。至于**常量宏**，绝大多数 C 项目用的是全大写的 `MAX_BUFFER_SIZE` 风格，而不是 PascalCase——这一点和 C# / Java 的习惯不同。
...
typedef unsigned int u32;              // 自己起名时带个前缀，避免冲突
...
5. **C 程序的 8 个翻译阶段**：字符映射 → 删除反斜杠换行 → 切成预处理记号 → 预处理 → 转成执行字符集 → 拼接相邻字符串 → 语法语义分析并翻译 → 链接。注意三字符组在 C23 已被**彻底删除**。
...
10. **保留标识符 5 条规则**：双下划线、下划线加大写、单下划线全局、标准库头文件名、以 E 开头的错误码宏，都是雷区，不要踩。

11. **命名规范**：snake_case（推荐）、camelCase、PascalCase。风格要统一，项目里不能混用。
