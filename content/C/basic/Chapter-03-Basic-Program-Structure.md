...
fatal error: my_missing_header.h: No such file or directory
    | #include "my_missing_header.h"
      ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
compilation terminated.
```

编译器内心的台词是：**"我去哪儿都找不到你说的这个文件，你是不是打错字了？或者路径写错了？"** 总之，乖乖去检查文件名和路径吧。
...
`void` 在函数参数列表里表示"这个函数不接受任何参数"。在 C 语言里，不写 `void` 也是合法的（比如 `int main()`），但含义可不一样——`int main()` 意思是"参数个数和类型未知"（C语言里空参数默认就是如此），而 `int main(void)` 明确表示"零参数"。两者在标准 `main` 函数场景下通常都能跑，但**显式写 `void` 是更好的习惯**，也是最规范的做法。
...
> 🚨 **血的教训**：如果你去面试，面试官看到你写 `void main()`，可能直接在心里给你扣 50 分——这属于"C 语言驾照扣满12分"级别的操作。养成好习惯，从今天起只用 `int main(void)`！
...
int main(void) {
    ...
    printf("Hello\rWorld\n");     // 回车，光标回到行首，World 覆盖 Hello
                                  // 终端实际看到: "World"（Hello 被覆盖了）
...
> 💡 编译器在处理 `"ZhangSan"` 时，偷偷在末尾多加了一个字节，存的就是 `\0`。所以 `strlen("ZhangSan")` 返回 8（不数结尾的 `\0`），但 `sizeof("ZhangSan")` 返回 9（数上了）——这哥俩差的那个就是 `\0` 在搞事情！
...
如果你用古老的 **C89/ANSI C** 标准编译，早期编译器可能直接报错（"不认识这是个啥玩意儿"），而较新的编译器则可能把 `//` 当作两个 `/` 运算符，和后面凑成一个 `/`，导致代码逻辑完全乱套：

```c
int x = 10; // 这是注释  <-- 在 C89 下，// 被当成 / 运算符，后面的 "这是注释  <-- ..." 被当成除法表达式，编译直接GG
```
...
> 📌 标准 C 定义了 **8 个翻译阶段**，阶段1（字符映射）和阶段2（续行处理）在这里合并展示，方便理解。阶段3的三字符组替换属于比较"冷门"的知识点，简单了解即可。

### 3.5.1 字符映射与换行符标准化

**阶段1**：把源文件里的所有字符映射到**源字符集**（source character set）。在这个阶段，文本行被拆解成独立的字符序列。

**阶段2**：**续行处理**在此完成——每行末尾的反斜杠加换行符（`\<换行>`）被"拼接"成一行，这是物理行到逻辑行的转换。

**阶段3（补充）**：三字符组（trigraphs）在**预处理阶段之前**的适当时候被替换（`??=` → `#`、`??/` → `\` 等）。
...
三字符组是 **C89/C90**（1990年）标准引入的稀奇古怪的东西，目的是照顾那些键盘上没有某些特殊字符的设备。简单说，就是用三个 ASCII 字符的组合来代表一个"不好直接输入"的字符。
...
> 😂 举个例子：`??=` 会被替换成 `#`，所以 `??=include <stdio.h>` 会被当成 `#include <stdio.h>`。听起来很荒谬？确实！**C99 将其废弃（deprecated）**，虽然 C23 仍保留其识别能力（但会发出诊断消息）。不过现代 C 代码里你不需要（也不应该）使用三字符组，了解一下"历史上还有这种操作"就够了。

### 3.5.3 预处理：宏展开、`#include` 插入

**阶段4** 是**预处理**（preprocessing），这是编译前最重要的一步。
...
**阶段5** 和 **阶段6**：经过预处理的代码被交付给**编译器**（compiler）。编译器进行语法分析、语义分析，生成**翻译单元**（translation unit）。同时执行各种诊断检查（比如"这行语法有问题"）。
...
> 💡 强烈建议把 `-Werror` 带上！有了它，任何警告都会直接导致编译失败，逼着你把每个警告都修掉。听起来很烦人？等你因为忽略了一个 `-Wconversion` 警告而在生产环境里算错了用户的银行余额，就知道它的好了。很多大型项目的 CI/CD 流水线都是这么配置的——警告就是警告，不处理就别想代码入库。
...
```bash
gcc -std=c11 myprogram.c -o myprogram    # 使用 C11 标准（目前主流）
gcc -std=c17 myprogram.c -o myprogram    # 使用 C17 标准（C11 的小补丁版，差异不大）
gcc -std=c23 myprogram.c -o myprogram    # 使用 C23 标准（最新，引入 nullptr 等新特性）
gcc -std=c99 myprogram.c -o myprogram    # C99 太老了，除非维护祖传代码否则别用
```
...
从 C99 开始，你可以用 Unicode 通用字符名（Universal Character Name）来命名标识符，这样就可以用中文、日文等语言命名变量了！语法是 `\uXXXX`（4位十六进制）或 `\UXXXXXXXX`（8位十六进制）。

> ⚠️ **但要注意**：C99 并不是"随便什么 Unicode 字符都能用"，而是有限制的——只有被纳入允许列表的 Unicode 字符才能作为标识符（具体规则涉及 Unicode Categories）。而且，用 `\uXXXX` 这种转义形式写中文变量名……说实话，挺丑的，不如直接写 UTF-8 中文变量名来得优雅！
...
### C23 增强：XID_Start 和 XID_Continue 规则，以及 `nullptr` 关键字

C23 采用了 Unicode 的 **XID_Start** 和 **XID_Continue** 属性来确定哪些字符可以作为标识符的开头字符和后续字符。这是一个更标准、更国际化的规则。

C23 还引入了 **`nullptr`** 关键字（类似 C++），用于表示空指针常量——这是对长期以来只能用 `NULL` 宏的一大改进，妈妈再也不用担心你把 `NULL` 和 `0` 搞混了。

简单来说，只要一个 Unicode 字符被标记为"可以用作编程语言的标识符"，C23 就支持。理论上，全世界各种语言的字符都可以作为标识符。

### 标识符的长度

C 标准不规定标识符的最大长度，但编译器通常有实现限制（GCC/Clang 至少支持 63 个字符外部链接，局部标识符几乎无限制）。`sizeof` 是运算符而非宏，和这些限制是两码事，日常编程基本不用操心。
...
8. **标识符规则**：C99 支持 Unicode 通用字符名（但有限制），C23 采用 XID_Start/XID_Continue 规则，可以轻松使用中文变量名。

9. **`nullptr`（C23）**：C23 引入了 `nullptr` 关键字表示空指针，比传统的 `NULL` 宏更清晰、更类型安全。
