...
首先纠正一个非常常见的错误说法：**`__func__` 本身就是字符串**，不是"需要加 `#` 才能变成字符串的标识符"。

标准规定 `__func__` 就像在函数开头写了 `static const char __func__[] = "函数名";`——它的类型就是字符数组，直接 `printf("%s", __func__)` 就行。
...
> ⚠️ **`#__func__` 是非法的**。`#`（字符串化运算符）**只能在宏定义体里使用**，写在普通代码里会直接报语法错误（`expected expression`）：
>
> ```c
> printf("%s\n", #__func__);   // ❌ error: expected expression
> ```
>
> 而且即使写成宏 `#define STR(x) #x`，`STR(__func__)` 得到的也是字面文本 `"__func__"`，**不是**函数名——因为字符串化发生在预处理阶段，而 `__func__` 是编译阶段才存在的预定义标识符。想拿到函数名，正确做法就是**直接用 `__func__`**。

> 💡 既然 `__func__` 已经是个数组了，那它能不能取地址、能不能拼字符串？都可以（它是 `static const char[]`，有确定地址）。但它**不能被赋值**，因为是 `const`。

如果你确实想在宏里"把记号变成字符串"，那要用的是字符串化运算符配合**宏参数**，例如：

```c
#include <stdio.h>

#define STR(x) #x          /* 字符串化：把宏参数原样变成字符串 */
#define XSTR(x) STR(x)     /* 二级展开：先展开宏参数，再字符串化 */

#define VERSION 42

int main(void) {
    printf("%s\n", STR(hello));      // 输出: hello
    printf("%s\n", STR(VERSION));    // 输出: VERSION（不展开！）
    printf("%s\n", XSTR(VERSION));   // 输出: 42（二级展开后才生效）
    return 0;
}
```
...
> 📦 **运行前提**：这段代码要求当前目录下确实存在 `sample.bin`（可以用上面那段小程序先生成），否则 `#embed "sample.bin"` 会直接报错找不到文件。同时 `#embed` 需要 **GCC 15 / Clang 19 或更新版本**才能编译，老编译器上要改用 `xxd -i` 之类的工具先生成数组。
...
int main() ??<
    printf("Hello??/n");  // 等价于 printf("Hello\n");
    return 0;
??>
```

> ⚠️ **这段代码在现代编译器上编译不过，请只当历史资料看**：
>
> - **GCC**：从 GCC 3.x 起默认就是关闭的，需要显式加 **`-trigraphs`** 才会识别。而在 **GCC 15 默认的 C23 模式下，已经彻底不支持**（C23 移除了三字符组）。
> - **Clang**：从来没有默认开启过三字符组支持，这段代码会直接报一堆莫名其妙的语法错误。
>
> 想亲手验证，请用 `gcc -std=c17 -trigraphs` 这种老标准加显式开关的组合。
>
> `??/` 代表反斜杠 `\`，所以 `??/n` 会被当成 `\` + `n`，也就是换行符。更"坑"的是：因为 `??/` 变成 `\` 发生在**阶段 1**（比续行处理还早），所以在老标准里 `??/` 出现在行尾时会把两行**悄悄拼接起来**——这是三字符组最著名的"事故来源"，也正是 C23 下定决心删掉它的原因之一。
