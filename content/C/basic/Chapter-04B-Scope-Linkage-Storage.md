...
> 有时候，我们需要在内层作用域里**明确地访问全局变量**，而不是被遮蔽的局部变量。这时候可以用**作用域解析运算符** `::`（在 C 语言中，我们通常用别的方法，详见下文），或者通过其他手段。
...
// struct Dog 和 enum Animal 不冲突，因为标签类型不同
// 但如果写成 enum Point 就会冲突（标签名字空间共享）
struct Dog {
    char *name;
    int age;
};
...
int main(void) {
    ...
    // enum GREEN 的值其实是 2，但这个名字已经被宏遮蔽了
    return 0;
}
```
...
**线程存储期**是 C11 引入（C23 正式成为关键字 `thread_local`）的一种存储期，专门用于**多线程程序**。每个线程都有自己独立的变量副本，线程之间互不干扰。
...
> 历史说明：`thread_local` 在 C11 中作为关键字引入（当时还需要包含 `<threads.h>`），在 C23 中正式标准化。如果你使用的是 GCC 或 Clang，也可以使用 `__thread` 关键字，它在 C11 之前就得到了广泛支持。
