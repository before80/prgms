...
### Enable state preservation and restoration {#Enable-state-preservation-and-restoration}
...
### Use scene storage {#Use-scene-storage}
...
### Restore the app state with an activity object {#Restore-the-app-state-with-an-activity-object}
...
1. Tap a product (“Cherries”) in the collection view to navigate to its detail information.
1. Pull down the system sheet from the top of the screen (to force Spotlight to update its index and request the activity). Note that the `DetailView` `userActivity` closure is called by iOS.
1. Go back to the app, and go back to the collection view.
1. Tap a product other than Cherries (i.e. Mango).
1. Suspend the app by tapping the Home button.
1. At the Home screen, swipe downwards to open the Spotlight window.
1. In the Spotlight search field, type “Cherries”. The search results will show “Show Cherries Product”.
1. Tap it. Note that `DetailView` `onContinueUserActivity` closure is called. The `DetailView` will show the Cherries product.
