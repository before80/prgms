...
Displaying a collection of data in a vertical list is a common requirement in many apps. Whether it’s a list of contacts, a schedule of events, an index of categories, or a shopping list, you’ll often find a use for a [List](https://developer.apple.com/documentation/swiftui/list).
...
> Note: If you want to remove the platform-appropriate styling — such as row separators or automatic disclosure indicators — from your list, consider using [LazyVStack](https://developer.apple.com/documentation/swiftui/lazyvstack) instead. For more information on working with lazy stacks, see [Creating performant scrollable stacks](../../1-LayoutFundamentals/1.4-CreatingPerformantScrollableStacks/).
...
### Prepare your data for iteration {#Prepare-your-data-for-iteration}

The most common use of [List](https://developer.apple.com/documentation/swiftui/list) is for representing collections of information in your data model. The following example defines a `Person` as an [Identifiable](https://developer.apple.com/documentation/swift/identifiable) type with the properties `name` and `phoneNumber`. An array called `staff` contains two instances of this type.
...
To present the contents of the array as a list, the example creates a `List` instance. The list’s content builder uses a [ForEach](https://developer.apple.com/documentation/swiftui/foreach) to iterate over the `staff` array. For each member of the array, the listing creates a row view by instantiating a new [Text](https://developer.apple.com/documentation/swiftui/text) that contains the name of the `Person`.
...
![A screenshot of a list view, showing a vertical list with two text entries: Juan Chavez and Mei Chen.](./images/Displaying-Data-in-Lists-plain-cells@2x.png)
...
### Display data inside a row {#Display-data-inside-a-row}

Each row inside a [List](https://developer.apple.com/documentation/swiftui/list) must be a SwiftUI [View](https://developer.apple.com/documentation/swiftui/view). You may be able to represent your data with a single view such as an [Image](https://developer.apple.com/documentation/swiftui/image) or [Text](https://developer.apple.com/documentation/swiftui/text) view, or you may need to define a custom view to compose several views into something more complex.
...
![A list with two entries, each showing a name and phone number. Each list item has a two-line cell. The first line shows the name in bold. The second shows a phone icon, followed by the phone number with a plain font and a lighter color.](./images/Displaying-Data-in-Lists-custom-cells@2x.png)

For more information on composing the types of views commonly used inside list rows, see [Building layouts with stack views](../../1-LayoutFundamentals/1.2-BuildingLayoutsWithStackViews/).

### Represent data hierarchy with sections {#Represent-data-hierarchy-with-sections}

[List](https://developer.apple.com/documentation/swiftui/list) views can also display data with a level of hierarchy, grouping associated data into sections.
...
Use [Section](https://developer.apple.com/documentation/swiftui/section) views to give the data inside a `List` a level of hierarchy. Start by creating the `List`, using a [ForEach](https://developer.apple.com/documentation/swiftui/foreach) to iterate over the `company.departments` array, and then create `Section` views for each department. Within the section’s view builder, use a `ForEach` to iterate over the department’s `staff`, and return a customized view for each `Person`.
...
![A list with sections. The first has a section header with the title Sales, followed by four person entries as individual rows in this section. The next section, Engineering, contains four entries, followed by Customer Service with two entries.](./images/Displaying-Data-in-Lists-sections@2x.png)

> Note: If your data hierarchy is too deep to represent with a single level of sections and rows, [OutlineGroup](https://developer.apple.com/documentation/swiftui/outlinegroup) and [DisclosureGroup](https://developer.apple.com/documentation/swiftui/disclosuregroup) might be a better fit. These views use a disclosure metaphor to allow someone to drill down to an arbitrary depth in the hierarchy.

### Use Lists for Navigation {#Use-Lists-for-Navigation}

Using a [NavigationLink](https://developer.apple.com/documentation/swiftui/navigationlink) within a [List](https://developer.apple.com/documentation/swiftui/list) contained inside a [NavigationStack](https://developer.apple.com/documentation/swiftui/navigationstack) adds platform-appropriate visual styling for navigation. SwiftUI navigates to a destination view you provide when a person chooses a list item.
...
For more information about navigation stacks, see [Understanding the navigation stack](../../../appstructure/6-Navigation/6.1-UnderstandingTheNavigationStack/).
