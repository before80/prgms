...
After you’ve created a working document-based app, you can extend it to handle scenarios that go beyond basic reading and writing. Support file formats that the system doesn’t know about by default by using a custom [UTType](https://developer.apple.com/documentation/uniformtypeidentifiers/uttype). Access files outside the normal read and write lifecycle using [makeFileCoordinator()](https://developer.apple.com/documentation/swiftui/urldocumentconfiguration/makefilecoordinator()), for example, to open a specific subfile in a package on demand. Add progress reporting to give people feedback during long operations.

> Note: If you’re new to document-based apps in SwiftUI, start with [Creating a document-based app](../5.1-CreatingADocumentBasedApp/).

## Declare a custom file format {#Declare-a-custom-file-format}
...
Then reference your type from the document’s [readableContentTypes](https://developer.apple.com/documentation/swiftui/readabledocument/readablecontenttypes) and [writableContentTypes](https://developer.apple.com/documentation/swiftui/writabledocument/writablecontenttypes), like this:
...
## Access files outside read and write {#Access-files-outside-read-and-write}
...
> Important: Always use [makeFileCoordinator()](https://developer.apple.com/documentation/swiftui/urldocumentconfiguration/makefilecoordinator()) for disk access outside of `read` and `write`. Accessing the file URL directly risks corruption because other processes, including iCloud, can change it any time.
...
Both [DocumentReader](https://developer.apple.com/documentation/swiftui/documentreader) and [DocumentWriter](https://developer.apple.com/documentation/swiftui/documentwriter) receive a [Subprogress](https://developer.apple.com/documentation/foundation/subprogress) parameter via their `read` and `write` methods, respectively. Report progress through the parameter so SwiftUI can display the appropriate UI during long operations.

Create a [ProgressReporter](https://developer.apple.com/documentation/foundation/progressreporter) from the [Subprogress](https://developer.apple.com/documentation/foundation/subprogress) by specifying a total unit count. Then call [complete(count:)](https://developer.apple.com/documentation/foundation/progressreporter) as the work finishes, like this:
...
If you use [FileHandle](https://developer.apple.com/documentation/foundation/filehandle) or [OutputStream](https://developer.apple.com/documentation/foundation/outputstream) to write files in chunks — for example, a large media file — report progress after each chunk rather than only once at the end. Each call to [complete(count:)](https://developer.apple.com/documentation/foundation/progressreporter) moves the progress bar forward, so spread the calls throughout the write.
...
The chunk size adapts to the file size, so large files don’t generate unnecessary system overhead. If you stream to an [OutputStream](https://developer.apple.com/documentation/foundation/outputstream) instead, the same pattern applies: Open the stream, write chunks of the computed size, call [complete(count:)](https://developer.apple.com/documentation/foundation/progressreporter) with the bytes written, and call `ProgresManager.complete(count:)` when finished.
