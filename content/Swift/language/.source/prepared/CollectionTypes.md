...
> For more about generic types and collections, see [Generics](../2.24-Generics/).
...
You can include a comma after last value in an array literal, which is known as a *trailing comma*:
...
Because the trailing comma makes every line end the same way, it's a useful way to write arrays like the one above that have one value per line. When you change the array, you only need to add, remove, or reorder values --- you don't have to add or remove commas.
...
For more about the `for`-`in` loop, see [For-In Loops](../2.5-ControlFlow/#For-In-Loops).
...
All of Swift's basic types (such as `String`, `Int`, `Double`, and `Bool`) are hashable by default, and can be used as set value types or dictionary key types. Enumeration case values without associated values (as described in [Enumerations](../2.8-Enumerations/)) are also hashable by default.
...
> For information about conforming to protocols, see [Protocols](../2.23-Protocols/).
...
### Creating and Initializing an Empty Set {#Creating-and-Initializing-an-Empty-Set}
...
### Fundamental Set Operations {#Fundamental-Set-Operations}
...
Like array literals, dictionary literals can include a trailing comma after the last key-value pair:
