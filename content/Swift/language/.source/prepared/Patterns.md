...
## Type-Casting Patterns {#Type-Casting-Patterns}
...
For an example that uses a `switch` statement to match values with `is` and `as` patterns, see [Type Casting for Any and AnyObject](../../languageguide/2.20-TypeCasting/#Type-Casting-for-Any-and-AnyObject).
