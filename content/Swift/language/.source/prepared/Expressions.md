...
For information about the behavior of these operators, see [Basic Operators](../../languageguide/2.2-BasicOperators/) and [Advanced Operators](../../languageguide/2.29-AdvancedOperators/).
...
For more information about in-out parameters and to see an example, see [In-Out Parameters](../../languageguide/2.6-Functions/#In-Out-Parameters).

In-out expressions are also used when providing a non-pointer argument in a context where a pointer is needed, as described in [Implicit Conversion to a Pointer Type](./#Implicit-Conversion-to-a-Pointer-Type).
...
For more information and to see examples of how to use `try`, `try?`, and `try!`, see [Error Handling](../../languageguide/2.17-ErrorHandling/).
...
For an example that uses the ternary conditional operator, see [Ternary Conditional Operator](../../languageguide/2.2-BasicOperators/#Ternary-Conditional-Operator).
...
For more information about type casting and to see examples that use the type-casting operators, see [Type Casting](../../languageguide/2.20-TypeCasting/).
...
A conditional expression appears only in the following contexts:
  ...
  - As the value assigned to a variable. - As the initial value in a variable or constant declaration. - As the error thrown by a `throw` expression. - As the value returned by a function, closure, or property getter. - As the value inside a branch of a conditional expression.
...
The *parameters* have the same form as the parameters in a function declaration, as described in [Function Declaration](../3.6-Declarations/#Function-Declaration).
...
For more information about escaping closures, see [Escaping Closures](../../languageguide/2.7-Closures/#Escaping-Closures).
...
For more information and examples of closure expressions, see [Closure Expressions](../../languageguide/2.7-Closures/#Closure-Expressions). For more information and examples of capture lists, see [Resolving Strong Reference Cycles for Closures](../../languageguide/2.26-AutomaticReferenceCounting/#Resolving-Strong-Reference-Cycles-for-Closures).
...
### Macro-Expansion Expression {#Macro-Expansion-Expression}
...
You use macro expressions to call freestanding macros. To call an attached macro, use the custom attribute syntax described in [Attributes](../3.7-Attributes/). Both freestanding and attached macros expand as follows:
...
A class, structure, or enumeration type can enable syntactic sugar for function call syntax by declaring one of several methods, as described in [Methods with Special Names](../3.6-Declarations/#Methods-with-Special-Names).

#### Implicit Conversion to a Pointer Type {#Implicit-Conversion-to-a-Pointer-Type}
...
Types declared with the `dynamicMemberLookup` attribute include members that are looked up at runtime, as described in [Attributes](../3.7-Attributes/).
...
For information about subscript declarations, see [Protocol Subscript Declaration](../3.6-Declarations/#Protocol-Subscript-Declaration).
