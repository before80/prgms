...
For more about these capabilities, see [Properties](../2.10-Properties/), [Methods](../2.11-Methods/), [Initialization](../2.14-Initialization/), [Extensions](../2.22-Extensions/), and [Protocols](../2.23-Protocols/).
...
The syntax used in the examples above marks the enumeration as conforming to the [`CaseIterable`](https://developer.apple.com/documentation/swift/caseiterable) protocol. For information about protocols, see [Protocols](../2.23-Protocols/).
...
Just like in the switch statement earlier, the `productBarcode` variable is matched against the pattern `.qrCode(let productCode)` here. And as in the switch case, writing `let` extracts the associated value as a constant. For more information about `if`-`case` statements, see [Patterns](../2.5-ControlFlow/#Patterns).
...
Here, the raw values for an enumeration called `ASCIIControlCharacter` are defined to be of type `Character`, and are set to some of the more common ASCII control characters. `Character` values are described in [Strings and Characters](../2.3-StringsAndCharacters/).
...
> For more information, see [Failable Initializers](../../languagereference/3.6-Declarations/#Failable-Initializers).
