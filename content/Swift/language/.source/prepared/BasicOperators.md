...
This chapter describes the common operators in Swift. [Advanced Operators](../2.29-AdvancedOperators/) covers Swift's advanced operators, and describes how to define your own custom operators and implement the standard operators for your own custom types.
...
Unlike the arithmetic operators in C and Objective-C, the Swift arithmetic operators don't allow values to overflow by default. You can opt in to value overflow behavior by using Swift's overflow operators (such as `a &+ b`). See [Overflow Operators](../2.29-AdvancedOperators/#Overflow-Operators).
...
> For more information, see [Identity Operators](../2.9-ClassesAndStructures/#Identity-Operators).
...
For more about the `if` statement, see [Control Flow](../2.5-ControlFlow/).
...
For more about `for`-`in` loops, see [Control Flow](../2.5-ControlFlow/).
...
Note that the array contains four items, but `0..<count` only counts as far as `3` (the index of the last item in the array), because it's a half-open range. For more about arrays, see [Arrays](../2.4-CollectionTypes/#Arrays).
