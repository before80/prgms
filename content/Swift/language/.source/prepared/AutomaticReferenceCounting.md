...
The initializer for `City` is called from within the initializer for `Country`. However, the initializer for `Country` can't pass `self` to the `City` initializer until a new `Country` instance is fully initialized, as described in [Two-Phase Initialization](../2.14-Initialization/#Two-Phase-Initialization).
...
For more information about capture lists, see [Capture Lists](../../languagereference/3.4-Expressions/#Capture-Lists).
