...
## Overview {#Overview}
This command creates a source archive for the package release, optionally signs it, and finally [publishes the package release](../../../design/8.1-RegistryServerSpecification/#4.6.-Create-a-package-release) to the registry.

If authentication is required for package publication, package authors should [configure registry login](../../../dependencies/3.2-UsingSwiftPackageRegistry/#Registry-authentication) before running `publish`.
...
For more details on trust-on-first-use, see [Trust on First Use](../../../essentials/1.3-PackageSecurity/#Trust-on-First-Use).

#### Package release metadata {#Package-release-metadata}
Package authors can specify a custom location of the package release metadata file by setting the `--metadata-path` option of the `publish` subcommand. Otherwise, package manager looks for a file named `package-metadata.json` in the package directory.
...
Refer to the [registry specification](../../../design/8.1-RegistryServerSpecification/#4.2.2.-Package-release-metadata-standards) for any additional requirements.

#### Package signing {#Package-signing}
If a registry requires signing, package authors need to sign the package release by either setting the `signing-identity` (for reading from operating system's identity store such as Keychain in macOS), or `private-key-path` and `cert-chain-paths` (for reading from files) options of the `publish` subcommand. This allows Package manager to locate the signing key and certificate.

For more information on package signing, see [Signed packages from a registry](../../../essentials/1.3-PackageSecurity/#Signed-packages-from-a-registry).
