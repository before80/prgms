...
### Using the package-collection CLI {#Using-the-package-collection-CLI}
With the `swift package-collection` command-line interface, SwiftPM users can subscribe to package collections. Contents of imported package collections are accessible to any clients of [libSwiftPM](../../extendingpackagemanager/6.2-SwiftPMAsALibrary/).

`swift package-collection` has the following subcommands:
- [`add`](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.1-PackageCollectionAdd/): Add a new collection
- [`describe`](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.5-PackageCollectionDescribe/): Get metadata for a collection or a package included in an imported collection
- [`list`](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.4-PackageCollectionList/): List configured collections
- [`refresh`](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.3-PackageCollectionRefresh/): Refresh configured collections
- [`remove`](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.6-PackageCollectionRemove/): Remove a configured collection
- [`search`](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.2-PackageCollectionSearch/): Search for packages by keywords or module names within imported collections
...
Package collections can be created and published by anyone. The [swift-package-collection-generator](https://github.com/apple/swift-package-collection-generator) project provides tooling intended for package collection publishers:
...
All package collections must adhere to the [collection data format](./#Input-Format) for SwiftPM to be able to consume them. The recommended way to create package collections is to use [`package-collection-generate`](https://github.com/apple/swift-package-collection-generator/tree/main/Sources/PackageCollectionGenerator). For custom implementations, the data models are available through the [`PackageCollectionsModel` module](https://github.com/swiftlang/swift-package-manager/tree/main/Sources/PackageCollectionsModel).
...
### Add packages to the collection {#Add-packages-to-the-collection}
...
* `identity`: The [identity](../../design/8.1-RegistryServerSpecification/#3.6.-Package-identification) of the package if published to a registry. **Optional.**
...
### Add versions to a package {#Add-versions-to-a-package}
...
* `signer`: The signer of the package version. **Optional.** Refer to the [documentation](../../essentials/1.3-PackageSecurity/#Package-signing) on package signing for details.
...
### Version-specific manifests {#Version-specific-manifests}
Package collection generators should include data from the "default" manifest `Package.swift` as well as [version-specific manifest(s)](../../buildingpackages/2.4-SwiftVersionSpecificPackaging/#Version-specific-Manifest-Selection)
...
[Version-specific tags](../../buildingpackages/2.4-SwiftVersionSpecificPackaging/#Version-specific-tags-when-resolving-remote-dependencies) are not supported by package collections.

### Configuration File {#Configuration-File}
Configuration that pertains to package collections are stored in the file `~/.swiftpm/config/collections.json`. It keeps track of user's list of configured collections and preferences such as those set by the `--trust-unsigned` and `--skip-signature-check` flags in the [`package-collection add` command](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.1-PackageCollectionAdd/).
...
### Signing a collection {#Signing-a-collection}
[Sign package collections](../../essentials/1.3-PackageSecurity/#Signed-package-collections) to establish authenticity and protect their integrity. Doing this is optional. Users will be prompted for confirmation before they can add an [unsigned collection](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.1-PackageCollectionAdd/#Unsigned-package-collections). The signing certificate you use to sign a package collection must meet a list of [requirements](../../essentials/1.3-PackageSecurity/#Requirements-on-signing-certificate). If these requirements are not met, the package manager returns an error.

For more details on the security features Package manager implements, see [Package Security](../../essentials/1.3-PackageSecurity/).
...
- **Signature replacement**: Attackers may modify a collection then re-sign it using a different certificate, either pretend to be the same entity or as some other entity, and SwiftPM will accept it as long as the [signature is valid](../../swiftcommands/7.6-SwiftPackageCollectionCommands/7.6.1-PackageCollectionAdd/#Signed-package-collections).
...
Since certificate-pinning configuration is associated with web domains, it can only be applied to signed collections hosted on the web (i.e., URL begins with  `https://`) and does not cover those found on local file system (i.e., URL begins with `file://`).
